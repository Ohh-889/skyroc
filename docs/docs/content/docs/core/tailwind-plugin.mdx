...
<Callout type="info">
  这些类型原先住在独立的 `@skyroc/ui-types` 包里，其中 `ThemeColor` 与本包 `tokens.ts` 的
  `SemanticColorName` 是一字不差的两份定义。该包已并入这里，重复的那一份被删掉了。`ThemeSize`
  刻意不复用 `tokens.ts` 的 `FontSizeToken`：那是字号尺度（中档叫 `base`），这里是组件尺寸档位
  （中档叫 `md`），语义和取值都不一样。
</Callout>
