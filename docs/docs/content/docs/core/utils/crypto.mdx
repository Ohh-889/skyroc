...
<Callout type="warn">
  **它不是安全边界。** crypto-js 的 passphrase 模式走的是 OpenSSL `EVP_BytesToKey`（MD5 单轮派生），
  密钥强度取决于口令本身，**不适合**用来保护真正的机密。 它的合理用途是本地存储混淆——让 localStorage /
  缓存里的内容不可直接肉眼读取。 需要真实加密强度时请改用 Web Crypto（`crypto.subtle`）或服务端加密； 接口传输加密用
  [`@skyroc/service` 的信封加密](/docs/core/service#接口传输加密)。
</Callout>
...
```ts
type TryDecryptResult<T> = { ok: true; data: T } | { ok: false; reason: 'decrypt' | 'parse' };
```
