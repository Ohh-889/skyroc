...
<Callout type="warn">
  这两个类型依赖 DOM lib，所以住在 `./web` 而不是 [`./type`](/docs/core/utils/type)。React Native
  侧不要从这里 import——`@skyroc/form` 的根出口刻意不再转发 `FieldElement`，就是为了不把 DOM
  类型带进无 DOM lib 的环境。
</Callout>
