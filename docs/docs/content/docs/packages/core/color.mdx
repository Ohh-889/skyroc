...
只要色族本身（名字 + 11 档），不要 `main` / `match` / `colorMap` 这些匹配元信息时用它。
统一入口 `getColorPalette(color, 'recommended')` 内部走的就是这个函数：

```ts
function getRecommendedColorPaletteFamily(color: string): ColorPaletteFamily
```
...
`hex → 色名` 的查找表，由内置的 1500+ 条色名数据折叠而成，供 `getColorName` 做精确匹配时使用：

```ts
import { colorNameMap } from '@skyroc/color';

colorNameMap['#1677ff']; // 精确命中时返回色名，未收录则为 undefined
```

需要「最近匹配」而不是「精确命中」时用 [`getColorName`](#色名匹配)——它在表里找不到时会退回加权距离最小的那一条。
