...
预设**只提供规则与 theme，不注入色值**——`--primary`、`--primary-500`、`--color-text` 这些变量由 [`@skyroc/web-admin-theme`](/docs/packages/web/theme/admin-theme) 在运行时写到 `:root` 上。两者缺一不可。
...
```ts
import presetSoybeanAdmin, {          // default = presetSoybeanAdmin
  presetSoybeanAdmin,                 // 具名导出（同一个函数）
  allShortcuts,
  flexShortcuts,
  positionShortcuts,
  textShortcuts,
...
注意尺寸档位里**没有 `sm`**，从 `xs` 直接跳到 `md`——这是刻意与 `presetWind3` 的默认 `sm` 保持不冲突。
...
| 维度 | `@sa/uno-config` | [`@skyroc/tailwind-plugin`](/docs/packages/web/tailwind-plugin) |
| --- | --- | --- |
...
- [`@skyroc/web-admin-theme`](/docs/packages/web/theme/admin-theme) —— 本预设消费的 CSS 变量由谁写入
- [`@skyroc/tailwind-plugin`](/docs/packages/web/tailwind-plugin) —— 组件库侧的等价物
- [Admin 样式方案](/docs/packages/web/admin-styles) —— 应用层样式的整体组织
