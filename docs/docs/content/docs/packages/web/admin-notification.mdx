...
| 版本 | 0.1.0 |
| 依赖 | [`@skyroc/utils`](/docs/packages/core/utils/overview)、[`@skyroc/web-ui-antd`](/docs/packages/web/ui)、[`@skyroc/web-ui-compose`](/docs/packages/web/ui)、`clsx`、`dayjs` |
...
在 Admin 中它作为 [`AdminLayout`](/docs/packages/web/admin-layouts/slots) 的 `headerMiddleActions` 插槽传入：
...
队列基于 [`PriorityQueue`](/docs/packages/core/utils/priority-queue)，排序三级：**优先级** → **时间倒序** → **入队序号倒序**。
...
- [布局插槽](/docs/packages/web/admin-layouts/slots) —— 铃铛按钮挂在哪个插槽
