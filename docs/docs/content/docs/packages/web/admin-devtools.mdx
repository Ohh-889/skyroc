...
description: "把 TanStack Query / Router 与 Jotai 三套 DevTools 聚合进一个浮动面板的开发期调试包，生产构建完全不打包"
---
...
三套 DevTools 各自都会往页面右下角塞一个浮动按钮。这个包把它们收进一个 `TanStackDevtools` 壳里：Query 和 Router 作为壳的两个 plugin 面板，Jotai 单独渲染但共用同一份主题和一套位置配置。
...
//                ^^^ 生产环境是个 no-op 组件，整个包会被 tree-shaking 掉
...
静态部分写在 `globalConfig.devtools` 里，`theme` 在运行时跟着 [`useSettingsTheme`](/docs/packages/web/theme/admin-theme) 的 `darkMode` 合并进去：
...
- **懒加载到底**：包内部四个 DevTools 组件本身也都是 `lazy()`，Jotai 的样式表也在 lazy 工厂里才 import。
