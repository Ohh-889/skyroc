...
| `apps/admin-example/src/pages/(admin)/keep-alive-form.tsx` | 用于验证缓存行为的表单页面（`apps/admin` 中无此页面，`keepAlive` 机制本身两个应用一致）。 |
...
```text
apps/admin-example/src/pages/(admin)/keep-alive-form.tsx
```
