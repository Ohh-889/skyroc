...
| 退出登录后 tabs 或菜单状态残留 | `logout()`、`clearMenus()`、`cacheTabs()`                           |
...
const RouterProvider = memo(() => {
  const { logout, getHomeRoute, homeRoute, initAuth, isAuthInitialized, isLoggedIn, userInfo } = useAuth();
...
如果用户信息请求失败，`initAuth()` 返回 `null`。守卫会调用 `logout()`，再跳回登录页。
...
| 切换用户后看见旧菜单  | `logout()` 是否执行了 `queryClient.clear()` 和 `clearMenus()`。                                 |
