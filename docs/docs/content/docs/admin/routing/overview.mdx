...
`apps/admin` 是精简起步模板，`pages` 下的文件很少，正好可以完整列出：

| 路径 | URL 表现 | 职责 |
| --- | --- | --- |
| `__root.tsx` | 根路由，不直接产生业务 URL | 全局 root route，挂载 loading、error、notFound、NProgress 和文档标题，并只在已有登录态时做认证预热。 |
| `index.tsx` | `/` | 根路径重定向：已登录去 `context.getHomeRoute()`，未登录去 `/login`。 |
| `error.tsx`、`not-found.tsx`、`loading.tsx` | 不产生 URL | 被插件的 `routeFileIgnorePattern` 排除，作为组件供 `__root.tsx` 引用。 |
| `(admin)/layout.tsx` | 不直接出现在 URL 中 | 后台 layout route，渲染 `WebAdminLayout` 并执行 `guardAdminRoute()`。 |
| `(admin)/home/index.tsx` | `/home` | 后台首页，模板中唯一的业务页面。 |
| `(auth)/login/layout.tsx` | `/login` | 登录页 layout route，`validateSearch` 校验 `redirect`，已登录时回到 redirect 或首页。 |
| `(auth)/login/index.tsx` | `/login` | 账号密码登录表单。 |
| `(auth)/login-out.tsx` | `/login-out` | 退出登录。 |
| `(errors)/403.tsx`、`404.tsx`、`500.tsx` | `/403`、`/404`、`/500` | 顶层错误页。 |

`(admin)`、`(auth)`、`(errors)` 是路由组。它们用于组织目录和 layout 关系，不会出现在 URL 中。

> 下文举例用到的 `manage/user`、`manage/user/$id`、`exception/*`、多级 `layout.tsx` 等页面，在 `apps/admin` 里并不存在——它们在 `apps/admin-example` 中有完整实现。写新页面时可以对照那边的目录结构。
...
认证相关页面放在 `(auth)` 下。`apps/admin` 当前只有账号密码登录（`login/index.tsx`）和退出登录（`login-out.tsx`）；注册、验证码登录、重置密码在 `apps/admin-example` 的 `(auth)/login/` 下（`register.tsx`、`code-login.tsx`、`reset-pwd.tsx`）。

`pages/(auth)/login/layout.tsx` 不负责认证初始化，只在用户已登录时做跳转兜底：
...
`pages/(errors)/403.tsx`、`404.tsx`、`500.tsx` 生成顶层 `/403`、`/404`、`/500`。`apps/admin-example` 里还额外有 `(admin)/exception/*` 页面，用于菜单中的异常页演示。两者职责不同：
...
动态参数用 `$` 前缀文件名表示。`apps/admin-example` 中的例子：
