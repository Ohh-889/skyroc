...
除了页面路由，`features/menus/menu-config.ts` 导出的 `menuNodeCallback` 还能给某个 layout 追加纯菜单节点（divider、group 标题等）。`apps/admin` 的实现是空的：

```ts
// apps/admin/src/features/menus/menu-config.ts
export const menuNodeCallback: MenuNodeCallback = () => {
  return [];
};
```

`apps/admin-example` 里给 `(admin)` 顶层菜单插了两条分隔线，可以作为写法参照：

```ts
const adminTopLevelMenuNodes = [
  { id: 'admin-feature-divider', menu: { order: 6, type: 'divider' } },
  { id: 'admin-about-divider', menu: { order: 20, type: 'divider' } }
...
回调参数是 route id，所以要先判断当前 layout 是不是目标分类。因此，顶层菜单 order 不只是页面路由之间排序，也会和这些扩展节点一起排序。
...
标准徽标用 `menu.badge`。`apps/admin-example` 的首页用了动态值（`apps/admin` 的首页没有配 badge）：

```ts
// apps/admin-example/src/pages/(admin)/home/index.tsx
...
当标准徽标不够用时，可以使用 `menu.extra`。`apps/admin` 的 `features/menus/extras.ts` 是空注册表：

```ts
// apps/admin/src/features/menus/extras.ts
export const menuExtras = {};

export type ExtraKey = keyof typeof menuExtras;
```

`apps/admin-example` 注册了两个扩展组件：

```ts
// apps/admin-example/src/features/menus/extras.ts
export const menuExtras = {
...
如果页面用 `validateSearch` 做了 search 校验（参考 [表格与表单](../features/table-and-form) 的 URL 同步），`query` 提供的默认值需要与该 schema 兼容。

## 外链和 iframe

### `href`：打开新窗口

`href` 表示这个路由菜单项点击后打开外部链接。`apps/admin-example` 的 `document/repository` 是这个模式：

```ts
// apps/admin-example/src/pages/(admin)/document/repository.tsx
...
注意组件返回 `null`——页面本身不渲染任何东西，跳转完全交给守卫。
...
`url` 是给页面组件消费的元信息。`apps/admin-example` 的 `document/react` 读取 route match 的 `staticData.url`：

```tsx
// apps/admin-example/src/pages/(admin)/document/react.tsx
...
`IframePage` 是 app 内组件（`features/router/components/IframePage`），不是共享包导出——`apps/admin` 里没有它，需要 iframe 页时自行实现或从 example 拷贝。
...
注意 `Extra` 与 `RoutePath` 是注册型字符串字面量：`Extra` 来自应用注册的 `MenuExtraRegistry`，`RoutePath` 来自生成的 `RoutePathRegistry`，因此 `extra` 和 `activeMenu` 在编译期能获得字符串提示与校验。
...
| 登录拦截和守卫链路 | [路由守卫](./guards) |
| 权限判定和 403 | [权限](./permission) |
