...
| 后台菜单中的异常演示 | `pages/(admin)/exception`（在 `apps/admin-example`） |
...
| `apps/admin-example/src/pages/(admin)/exception` | 后台壳内的异常页示例。`apps/admin` 只有顶层 `(errors)` 错误页。 |
...
顶层错误页在 `pages/(errors)`，两个应用都有。后台异常演示页 `pages/(admin)/exception` 只在 `apps/admin-example` 里，它会经过后台 layout 和权限守卫。
