...
| 工具      | 要求                        | 来源                  |
| --------- | --------------------------- | --------------------- |
| Node.js   | `^22.11 \|\| ^24 \|\| >=26` | 根 `package.json`     |
| pnpm      | `>=11`                      | 根 `package.json`     |
| workspace | 使用 pnpm workspace         | `pnpm-workspace.yaml` |

仓库不锁定 pnpm 的具体版本，但要求使用 pnpm 11 或更高版本。
...
开发服务默认监听 **`http://localhost:9527`**，`host` 为 `0.0.0.0`，启动后自动打开浏览器；`vite preview` 的预览端口是 **9725**。这两个默认值来自 `@skyroc/web-admin-vite`（`packages/web/admin-vite/src/config.ts`），可以在 `apps/admin/vite.config.ts` 的 `server.port` / `preview.port` 覆盖。
