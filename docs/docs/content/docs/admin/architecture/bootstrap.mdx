...
| 修改主题、暗色模式或 Ant Design 主题接入     | `setupTheme`、`features/antd/AntdProvider.tsx`、`ThemeEffect`（来自 `@skyroc/web-admin-theme`，挂在 `GlobalEffect` 里） |
...
开发环境会先加载 `@shell/devtools/jotai`，再进入真正的应用启动。这样 Jotai devtools 的接入发生在应用 atom 被 Provider 使用之前。生产环境不会加载这段 devtools 入口。
...
注意第三行来自共享包 [`@skyroc/web-admin-styles`](/docs/web/admin-styles)，不是应用本地样式文件——全局 CSS 资产是沉淀在包里的，应用只负责 import。
...
| `menuNodeCallback`    | 为顶层菜单追加 divider 等项目级菜单节点。`apps/admin` 里返回空数组，`apps/admin-example` 用它插了两条分隔线。                          |
| `extras`              | 注册标准 badge 之外的菜单扩展 UI。`apps/admin` 的 `menuExtras` 是 `{}`，`apps/admin-example` 注册了 `NewFeature` 和 `ReleaseChannel`。 |
...
| `RouterProvider`       | 把 `useAuth()` 返回的登录态、用户信息、首页和初始化/登出函数注入 TanStack Router context（`Router.RouterContext`）。 |
...
| 阶段                | 位置                                 | 行为                                                                                                                                   |
| ------------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| Router context 注入 | `features/router/RouterProvider.tsx` | 从 `useAuth()` 读取并注入 7 个字段：`initAuth`、`isAuthInitialized`、`isLoggedIn`、`userInfo`、`logout`、`getHomeRoute`、`homeRoute`。 |
...
| 开发环境 devtools 不出现 | `import.meta.env.DEV` 是否为 true，`main.tsx` 是否先加载了 `@shell/devtools/jotai`，`App.tsx` 中 `AdminDevtools` 是否被 lazy 加载。 |
