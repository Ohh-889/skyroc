...
| 排查 tabs 缓存 | `@skyroc/web-admin-layouts`、`useAuth().logout()`、`useInitLogin()` |
...
| `apps/admin/src/features/auth/use-auth.ts` | 登录态 atom、`setAuth()`、`initAuth()`、`logout()`。 |
| `apps/admin/src/features/auth/use-login.ts` | 登录成功后按 `lastLoginUserId` 判断是否清理 `globalTabs`。 |
| `apps/admin/src/features/auth/use-auth.ts` | `getToken()`、`clearAuthStorage()`。 |
...
| `lastLoginUserId` | `logout()` 退出时记录；`useInitLogin()` 登录成功后比较 | 用来判断是否同一用户再次登录。 |
...
不要只删除 `token`。退出登录必须走 `logout()`，否则用户信息、菜单、tabs 或 React Query 缓存可能残留。
...
| 退出登录只删 `token` | 调用 `logout()`，同时清 React Query、认证 storage、菜单和 tabs。 |
