...
| `apps/admin/src/components/SystemLogo.tsx`                            | 应用自己的品牌图标。                                                                      |
| `apps/admin/src/features/auth/components/UserAvatar.tsx`              | Header 右侧用户头像和用户操作入口。                                                       |
...
const GlobalContent = () => {
  ...
  return <div className="h-full grow bg-layout p-16px">{content ?? <Outlet />}</div>;
};
```
...
| 共享布局包完整 API            | [@skyroc/web-admin-layouts](/docs/web/admin-layouts/overview) |
