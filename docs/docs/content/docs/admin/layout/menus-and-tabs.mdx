...
| `useAuth().logout()` | 退出登录时先缓存当前 tabs，再清理菜单和认证状态。         |
...
| 退出登录时只清 token，不处理菜单和 tabs     | 当前 `logout` 会清 Query、认证存储、菜单，并缓存 tabs；用户切换时还会清 `globalTabs`。 |
...
| 权限和动态授权                                      | [权限](../routing/permission)                                                               |
| 共享布局包菜单与 tabs API                           | [菜单系统](/docs/web/admin-layouts/menus)、[状态与页签](/docs/web/admin-layouts/state-tabs) |
