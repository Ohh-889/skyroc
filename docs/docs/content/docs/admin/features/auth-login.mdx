...
> 只想查 `useAuth()` / `useInitLogin()` 及 `setAuth` / `getToken` 等的签名和返回值，直接看 [登录认证 API](./auth-login-api)。
...
| 修改登录页布局和动画         | `pages/(auth)/login/layout.tsx`（`modules/Header.tsx` 只在 `apps/admin-example` 里） |
| 接真实登录接口               | `service/api/auth/api.ts`、`AUTH_URLS.LOGIN`                                         |
| 调整 token 存储和刷新        | `features/auth/use-auth.ts`、`service/adapter.ts`                                    |
| 理解登录后为什么要初始化菜单 | `useAuth().initAuth()`、`useMenus().initMenus(data)`                                 |
| 实现退出登录                 | `pages/(auth)/login-out.tsx`、`useAuth().logout()`                                   |
...
| `apps/admin-example/src/pages/(auth)/login/code-login.tsx` | 验证码登录页面（`apps/admin` 中无此页面）。                                                                                                                        |
| `apps/admin-example/src/pages/(auth)/login/register.tsx`   | 注册页面入口（`apps/admin` 中无此页面）。                                                                                                                          |
| `apps/admin-example/src/pages/(auth)/login/reset-pwd.tsx`  | 重置密码页面入口（`apps/admin` 中无此页面）。                                                                                                                      |
| `apps/admin/src/pages/(auth)/login-out.tsx`                | 退出登录路由。组件返回 `null`，在 `beforeLoad` 里 `await context.logout()` 后 `redirect` 到 `/login`（必须等登出完成，否则 `/login` 守卫会读到还没清掉的 token）。 |
...
| `apps/admin/src/features/auth/use-auth.ts`                 | 登录态 hook，同时导出 `getToken()`、`clearAuthStorage()` 两个 helper。                                                                                             |
...
pages/(auth)/login/index.tsx
  ...
  -> useLoginMutation()
  -> fetchLogin()
  -> setAuth(data)  // Api.Auth.LoginToken
  -> initAuth()
  -> initMenus(userInfo)
  -> navigate(redirect 或 /)
```
...
| `pages/(auth)/login/code-login.tsx` | `/login/code-login` | 验证码登录入口（仅 example）。               |
| `pages/(auth)/login/register.tsx`   | `/login/register`   | 注册入口（仅 example）。                     |
| `pages/(auth)/login/reset-pwd.tsx`  | `/login/reset-pwd`  | 重置密码入口（仅 example）。                 |
...
| 在页面按钮里手动清理 token、query、菜单和 tabs | 导航到 `/login-out` 或复用 `logout()`。                                      |
...
- API 速查（useAuth / useInitLogin / 导出函数）：[登录认证 API](./auth-login-api)
