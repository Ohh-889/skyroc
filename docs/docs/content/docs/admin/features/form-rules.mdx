...
| 注册或重置密码校验     | `apps/admin-example` 的 `pages/(auth)/login/register.tsx`、`reset-pwd.tsx`                                   |
| 验证码登录             | `apps/admin-example` 的 `pages/(auth)/login/code-login.tsx`、`formRules.phone`、`formRules.code`             |
| 新增认证字段规则       | `features/auth/use-auth-form-rules.ts`、`@skyroc/utils` 正则常量                                             |
| 修改校验文案           | `packages/web/admin-i18n/src/langs/*/form.json`、`packages/@core/types/src/locales/i18n.d.ts` 的 `I18n.Form` |
...
| `packages/@core/types/src/locales/i18n.d.ts`          | `I18n.LocaleMessages.form` 约束表单文案 key 的类型。                        |
...
3. 在 `packages/@core/types/src/locales/i18n.d.ts` 的 `I18n.Form` 里增加类型。
