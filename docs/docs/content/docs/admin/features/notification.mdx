...
> 只想查 `NotificationProvider` props、`useNotificationContext()` 返回值和类型，直接看 [通知系统 API](./notification-api)。
...
| 做通知演示或调试               | `apps/admin-example` 的 `pages/(admin)/interaction/notification/index.tsx`、`useMockNotifications()` |
...
| `apps/admin-example/src/pages/(admin)/interaction/notification/index.tsx` | 通知系统演示页，展示不同类型、优先级、批量操作、权限和配置控制；用 `@skyroc/web-admin-notification/mock` 的 `useMockNotifications()` 造数据。 |
...
`apps/admin-example/src/pages/(admin)/interaction/notification/index.tsx` 是通知系统演示页（`apps/admin` 中无此页面）。它展示了：
...
在自己的应用里接通知中心时，把这一页当作 API 用法清单看即可：`NotificationProvider` 已经在 `App.tsx` 里挂好（两个应用一致），页面只要 `useNotificationContext()` 就能用；`useMockNotifications()` 只用于演示造数，接真实接口时整体替换掉。
...
- API 速查（Provider props / context 返回值 / 类型）：[通知系统 API](./notification-api)
