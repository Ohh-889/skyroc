...
> **本页描述 `apps/admin-example`。** `apps/admin` 的首页只是一块起步占位卡片，没有图表；`useEcharts()` 也是 example 的 app 内 hook（`apps/admin-example/src/hooks/use-echarts.ts`），不是共享包导出。要在自己的应用里做看板，把这个 hook 和 `home/modules/*` 拷过去即可。

这一页说明 Dashboard 的组成方式。重点不是讲 ECharts 全部 API，而是说明已经封装好的 `useEcharts()`、首页模块和菜单 badge 怎么协作。

example 的首页是一个真实可运行的 dashboard，但其中数据仍以静态或示例数据为主。接真实后端时，应把数据请求放在服务模块和页面模块之间，不要把请求、图表初始化、主题切换和 resize 全部揉在一个组件里。

## 适用场景

| 场景 | 重点位置 |
| --- | --- |
| 新增首页图表模块 | `apps/admin-example/src/pages/(admin)/home/modules` |
| 接入 ECharts 并跟随暗色模式 | `apps/admin-example/src/hooks/use-echarts.ts` |
...
| 文件 | 职责 |
| --- | --- |
| `apps/admin-example/src/pages/(admin)/home/index.tsx` | 首页页面组合，注册 `HOME_MENU_BADGE_KEY`，挂载卡片、折线图、饼图和项目动态模块。 |
| `apps/admin-example/src/pages/(admin)/home/modules/LineChart.tsx` | 折线图模块示例。同目录还有 `CardData.tsx`、`PieChart.tsx`、`HeaderBanner.tsx`、`CreativityBanner.tsx`、`ProjectNews.tsx`。 |
| `apps/admin-example/src/pages/(admin)/home/modules/PieChart.tsx` | 饼图模块示例。 |
| `apps/admin-example/src/hooks/use-echarts.ts` | ECharts 注册、初始化、主题切换、尺寸变化、销毁和 options 更新封装。这是 app 内 hook，不是共享包导出。 |
| `packages/web/admin-theme/src/hooks/use-theme.ts` | 图表读取的主题状态来源。 |
| `packages/web/admin-layouts/src/state/menus` | 菜单 badge 的运行时状态来源（`useAdminMenuBadges()`）。 |
...
| 层 | 示例位置 | 职责 |
| --- | --- | --- |
| 服务模块 | 新建 `service/api/dashboard` | 按 `urls.ts`、`api.ts`、`hooks.ts`、`keys.ts`、`index.ts` 五文件结构组织。当前两个应用都还没有这个模块，首页图表用的是本地静态数据。 |
