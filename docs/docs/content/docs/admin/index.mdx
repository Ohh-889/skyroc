...
## 两个应用：admin 与 admin-example

仓库里有**两个后台应用**，读文档前必须先分清，否则会在 `apps/admin` 里找不到文档提到的页面：

| 应用 | 定位 | 页面现状 |
| --- | --- | --- |
| `apps/admin`（`skyroc-admin`） | **精简起步模板**，二次开发的实际起点 | `pages/(admin)` 下只有 `home` 与 `layout`，外加登录页与错误页 |
| `apps/admin-example` | **完整示例应用**，演示所有能力 | `manage/{user,menu,role}` CRUD、`plugin/*`、`function/*`、`interaction/*`、`projects/*` 等上百个页面 |

两者共用同一套 `@skyroc/*` 共享包、同一套启动流程与路由机制，差别只在页面数量和 `service/api` 模块（`admin-example` 多一个 `system-manage`）。

因此本节文档的**分工**是：

- **启动、路由、布局、请求、主题、配置、部署**——以 `apps/admin` 为准，两个应用一致；
- **表格与表单、图表 Dashboard、示例页面边界、通知与反馈演示**——这些能力在 `apps/admin` 中没有落地页面，正文引用的是 `apps/admin-example`，页面顶部会再次标注。
...
| 请求服务      | `apps/admin/src/service` 按 `urls`、`api`、`hooks`、`keys`、`index` 分层，页面优先通过 React Query hooks 使用接口。     |
...
| 共享后台能力               | `packages/web/*`、`packages/@core/*`、[包总览](/docs/packages)                           |
