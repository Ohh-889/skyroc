...
| 你要做什么                  | 先看哪里                                                                                            |
| --------------------------- | --------------------------------------------------------------------------------------------------- |
| 新增路由标题或菜单文案      | `packages/web/admin-i18n/src/langs/*`、`packages/@core/types/src/locales/i18n.d.ts` 的 `I18n.Route` |
| 新增页面或业务文案类型      | `packages/@core/types/src/locales/i18n.d.ts` 和共享语言资源                                         |
...
| `packages/@core/types/src/locales/i18n.d.ts`       | 全局 `I18n` 命名空间，约束 `$t()`、`i18nKey` 和语言资源 key。                |
...
packages/@core/types/src/locales/i18n.d.ts
```
...
这里的 `report` 来自 `report.svg` 的文件名。`icon-local` 来自 `.env` 的 `VITE_ICON_LOCAL_PREFIX`，用于生成 SVG symbol id，例如 `#icon-local-report`。如果改了前缀，既要改 env，也要确认 `globalConfig.localIconPrefix`、`SvgIcon` 的 `localIconPrefix` 和 Vite 图标插件配置仍然一致。
...
1. 文案不显示时，先确认 `i18nKey` 是否在 `packages/@core/types/src/locales/i18n.d.ts` 中存在。
...
| 误区                                    | 正确做法                                                                                                                                               |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 只加翻译 JSON，不改类型                 | 同步 `packages/@core/types/src/locales/i18n.d.ts`，让 key 有类型约束。                                                                                 |
...
| 组件、菜单和后端动态菜单图标用法   | [图标使用](./icons)                              |
| 路由 `i18nKey`、菜单图标和隐藏菜单 | [路由元信息](../routing/route-meta)              |
| Iconify、本地 SVG 和 Vite preset   | [环境变量与 Vite](../configuration/env-and-vite) |
| 共享 i18n 包完整 API               | [@skyroc/web-admin-i18n](/docs/web/admin-i18n)   |
| 共享 Vite 图标配置                 | [@skyroc/web-admin-vite](/docs/web/admin-vite)   |
