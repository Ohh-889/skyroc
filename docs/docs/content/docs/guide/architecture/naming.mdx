...
举例：颜色 **算法**（OKLCH 调色板生成）放 `@skyroc/color`——它依赖 colord/culori，不符合 shared 准入。颜色 **token**（hex / hsl 常量）曾经放在 shared，现已并入 [`@skyroc/tailwind-plugin`](/docs/packages/web/tailwind-plugin)：三端统一走 Tailwind 后该插件是唯一消费者，令牌没有第二个读者，独立成包只剩形式意义。
