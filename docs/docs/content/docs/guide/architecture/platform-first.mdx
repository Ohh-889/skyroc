---
title: 平台优先目录
description: packages/web / packages/native / packages/shared / packages/primitives 的分工与共生
---
...
前四个是跨平台的，后三个是平台专属的。各目录下具体有哪些包、分别叫什么，见 [目录结构总览](/docs/guide/getting-started/directory)。
...
包（2 个）：`ui-types`（共享 TS 类型）、`type-utils`（高级 TS 工具类型）。

跨端 React hooks 不在 `shared/` 里，而是独立的 `packages/hooks`（`@skyroc/hooks`）——它依赖 React 运行时，不符合 shared 的准入门槛。

详见 [Shared 总览](/docs/packages/shared/overview)、[`@skyroc/hooks`](/docs/packages/hooks)。
...
详见 [Web 总览](/docs/packages/web/overview)。
