...
| 工具    | 要求                        | 说明                         |
| ------- | --------------------------- | ---------------------------- |
| Node.js | `^22.11 \|\| ^24 \|\| >=26` | 以仓库根 `package.json` 为准 |
| pnpm    | `>= 11`                     | 不锁定具体版本               |
...
如果本机还没有 pnpm，可以按 pnpm 官方安装方式安装最新版：
...
仓库不锁定 pnpm 的具体版本，但要求 pnpm 11 或更高版本。升级后重新执行 `pnpm install`，不要混用 npm 或 Yarn 生成依赖状态。
...
仓库已经通过 `pnpm-workspace.yaml` 的 `allowBuilds` 声明允许执行安装脚本的依赖。先确认 pnpm 版本和当前网络环境，再查看 [常见问题](/docs/guide/getting-started/faq) 中的安装排查说明。
