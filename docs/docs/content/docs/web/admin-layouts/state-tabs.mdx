...
```tsx
const { menus, firstLevelMenus, secondLevelMenus, childLevelMenus, selectedKey, openKeys, routerPushByKey } =
  useAdminMenus();
```
...
```tsx
const { activeTabId, allTabs, removeActiveTab, clearLeftTabs, clearRightTabs, clearOtherTabs } = useAdminTab();
```
