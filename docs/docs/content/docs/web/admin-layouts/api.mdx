...
| 导出                        | 类型      | 说明                                                                                        |
| --------------------------- | --------- | ------------------------------------------------------------------------------------------- |
| `AdminLayout`               | Component | 应用级后台布局壳。插槽细节见 [布局插槽](/docs/web/admin-layouts/slots)。                    |
| `setupAdminLayouts`         | Function  | 启动阶段注入布局全局配置。                                                                  |
| `useMenus`                  | Hook      | 初始化和读取菜单原始数据。                                                                  |
| `useAdminMenus`             | Hook      | 读取当前布局菜单视图和菜单交互方法。详见 [状态与页签](/docs/web/admin-layouts/state-tabs)。 |
| `useAdminState`             | Hook      | 读取和修改布局运行时状态。详见 [状态与页签](/docs/web/admin-layouts/state-tabs)。           |
| `useAdminTab`               | Hook      | 读取和操作路由页签。详见 [状态与页签](/docs/web/admin-layouts/state-tabs)。                 |
| `useAdminMenuBadges`        | Hook      | 读取和更新菜单 badge 动态值。详见 [状态与页签](/docs/web/admin-layouts/state-tabs)。        |
...
`AdminLayout` 的详细插槽说明见 [布局插槽](/docs/web/admin-layouts/slots)，这里不重复展开。
...
插槽行为见 [布局插槽](/docs/web/admin-layouts/slots)。
...
```tsx
const { clearMenus, getHomeRoute, home, initMenus, menus, quickReferenceMenus } = useMenus();
```
...
| `routeMode` | 行为                                                                                                                         |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `static`    | 从 `routeTree` 的 `staticData` 生成菜单。详见 [静态路由菜单生成](/docs/web/admin-layouts/static-menu-generation)。           |
| `dynamic`   | 调用 `loadDynamicRoutes`，从后端路由结构生成菜单。详见 [动态路由菜单生成](/docs/web/admin-layouts/dynamic-menu-generation)。 |
...
`useAdminMenus` 已在 [状态与页签](/docs/web/admin-layouts/state-tabs) 展开。它基于 `useMenus` 的原始数据，计算当前布局需要的菜单视图：
...
type MenuNodeCallback = (routeId: Router.RouteId) => MenuNodeConfig[];
```
