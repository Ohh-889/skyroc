...
- [静态路由菜单生成](/docs/web/admin-layouts/static-menu-generation)：适合前端路由文件声明菜单。
- [动态路由菜单生成](/docs/web/admin-layouts/dynamic-menu-generation)：适合后端返回路由树和菜单配置。
...
| 模式      | 数据来源                              | 适用场景                                               | 深入文档                                                            |
| --------- | ------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------- |
| `static`  | TanStack Router route 的 `staticData` | 路由和菜单由前端仓库管理，权限也由前端 meta 参与过滤。 | [静态路由菜单生成](/docs/web/admin-layouts/static-menu-generation)  |
| `dynamic` | `loadDynamicRoutes` 返回的后端路由树  | 菜单、首页和路由树由后端或权限中心下发。               | [动态路由菜单生成](/docs/web/admin-layouts/dynamic-menu-generation) |
