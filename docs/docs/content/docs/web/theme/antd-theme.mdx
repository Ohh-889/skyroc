...
```ts
function generateNeutralColorPalettes(bgBaseColor: string, textBaseColor: string): NeutralColorPalette;
```
...
```ts
function generateDarkNeutralColorPalettes(bgBaseColor: string, textBaseColor: string): NeutralColorPalette;
```
...
```ts
function genPaletteVars(colorName: string, palette: Record<number, string>): Record<string, string>;
```
