...
- `html` 的边框色、字号、文字色分别绑到 `--color-border`、`--font-size`、`--color-text-base`——这几个变量由 [`@skyroc/web-admin-theme`](/docs/web/theme/admin-theme) 在运行时写到 `:root` 上；
...
配合 [`@skyroc/web-admin-runtime`](/docs/web/admin-runtime) 的 `setupNProgress` 使用：JS 在那个包里，样式在这个包里。
...
| [`@skyroc/tailwind-plugin`](/docs/core/tailwind-plugin) | 提供 CSS 变量与设计 token，被下面两者消费 |
