...
| 层级     | 包                        | 职责                                                      |
| -------- | ------------------------- | --------------------------------------------------------- |
| 设计令牌 | `@skyroc/tailwind-plugin` | 语义色令牌（`src/tokens.ts`）+ 主题变量生成               |
...
<Cards>
  <Card
    title="主题系统"
    description="ConfigProvider、语义颜色、暗色模式与全局配置"
    href="/overview/theming"
  />
  <Card
    title="设计令牌"
    description="颜色、间距、圆角、字体的使用边界"
    href="/overview/design-system/tokens"
  />
  <Card
    title="组件总览"
    description="查看所有可用组件与 API"
    href="/components"
  />
</Cards>
