...
/**
 * 达到这个宽度才启用左右分栏。必须和 global.css 里 `.skyroc-split` 的断点保持一致，
 * 否则会出现「CSS 已分栏但预览还内联」的错位。
 */
...
interface DemoStageContextValue {
  /**
   * 是否启用左右分栏。
   * true：整页预览在右侧常驻，左侧只留代码；
   * false：没有右栏，每个 demo 把自己的预览渲染回代码上方。
   */
...
/**
 * 右侧粘性手机，渲染 playground 的整个组件页（NavBar + 整页 demo），内部自己滚动。
 * 它不跟随左栏滚动切换 —— 左边一节节读代码，右边始终是完整的真机效果。
 */
