...
interface DemoProps {
  ...
  description?: string;
  /**
   * Demo 定位，只支持 playground 协议： "@playground/ButtonDemo" → apps/native-ui-playground/src/demos/ButtonDemo.tsx
   *
   * 之所以不接受任意路径：预览是在浏览器里跑真实 RN 组件，模块必须能被 Turbopack 静态解析到 demos 目录（见 demo-preview.tsx），docs 侧临时写的散落文件进不了那张表。
   */
  src: string;
  /** 预览区标题，缺省用 demo 名 */
  title?: string;
...
/**
 * 映射到 playground 的 app/components/*.tsx 路由： 嵌套 demo 取目录名（"button/ButtonColor" → /components/button）， 顶层 demo 去掉 Demo
 * 后缀（"ButtonDemo" → /components/button）。
 */
    ...
    <DemoShell
      code={
        <ServerCodeBlock
          code={source.trimEnd()}
          lang="tsx"
          // 外层 figure 已有边框圆角和标题栏，代码块只留顶部分隔线，避免套两层框、顶两条标题
          codeblock={{ className: 'rounded-none border-0 border-t shadow-none' }}
        />
      }
