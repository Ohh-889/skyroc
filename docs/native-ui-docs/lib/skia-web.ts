 ...
 *
 * 原生端 Skia 的实现由 JSI 注入全局 `SkiaApi`，import 进来就能用；web 端不是：
 * `@shopify/react-native-skia` 的 `Skia.web.ts` 在**模块求值那一刻**执行
 * `JsiSkApi(global.CanvasKit)`，把当时的全局 CanvasKit 一次性捕获进每个工厂对象。
 * wasm 还没加载时 `global.CanvasKit` 是 undefined，`Skia` 依旧是个正常对象，
 * 但工厂里握着的全是 undefined，直到 `Skia.Path.Make()` 才炸成
 * `Cannot read properties of undefined (reading 'Path')`。
 *
 * 所以这里是个硬时序：**任何 import 到 @shopify/react-native-skia 的模块，
 * 都必须等 ensureSkiaWeb() resolve 之后才被求值**，事后再补加载是救不回来的。
 * 预览本来就是动态 import，在 import demo 之前 await 一下即可。
 */
...
/**
 * 用到 Skia 的组件 slug。CanvasKit 是 8MB 的 wasm，只有这些预览值得为它买单，
 * 其余组件页不该被拖慢，所以不做无差别预加载。新增基于 Skia 的组件时补一条。
 */
...
/**
 * 加载 CanvasKit 并挂到 `global.CanvasKit` 上 —— 这正是官方 LoadSkiaWeb 做的事，
 * 也是 Skia.web.ts 唯一依赖的约定。并发调用共用同一个 promise。
 */
