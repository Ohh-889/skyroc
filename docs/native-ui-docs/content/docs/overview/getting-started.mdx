...
<Callout type="warn">
  `@source` 的路径相对于 `global.css`，且必须指向**当前安装方式下真实存在的组件代码**——发布包扫描 `dist`，workspace
  软链接开发时扫描 `src`。指错了 Tailwind 就发现不了组件库内部用的 className，组件会渲染成没有样式的裸元素。仓库内的
  playground 用的是 `@source "./node_modules/@skyroc/native-ui/src"`。
</Callout>
