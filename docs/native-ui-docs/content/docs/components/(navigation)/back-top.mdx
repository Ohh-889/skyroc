...
`right` / `bottom` 是按钮到**屏幕物理边缘**的距离，`size` 是按钮直径：

| 属性     | 默认值 | 说明                             |
| -------- | ------ | -------------------------------- |
| `right`  | 30     | 距屏幕右边的距离                 |
| `bottom` | 128    | 距屏幕底边的距离，已预留 TabBar  |
| `size`   | 40     | 按钮直径，比 `FloatingButton` 的默认 48 小一号 |

位置按窗口尺寸计算，渲染却是相对父容器的绝对定位，因此 `BackTop` 必须挂在铺满屏幕、且左上角与屏幕左上角重合的容器里。页面上方还有 NavBar 之类的固定元素时，用 `Portal` 把它挂到页面根节点（示例里就是这么做的）。
...
<PropsTable
  data={[
    ...
    { name: 'right', description: '距屏幕右边的距离（像素）', type: 'number', default: '30' },
    {
      name: 'bottom',
      description: '距屏幕底边的距离（像素），不含安全区，需要避开 home indicator 或 TabBar 时自行并入',
      type: 'number',
      default: '128'
    },
    { name: 'size', description: '按钮直径（像素）', type: 'number', default: '40' },
    { name: 'immediate', description: '为 true 时瞬间跳到顶部，为 false 时走滚动动画', type: 'boolean', default: 'false' },
