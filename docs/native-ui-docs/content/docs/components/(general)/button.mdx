...
```ts
import type { ButtonColor, ButtonProps, ButtonShape, ButtonSize, ButtonSlots, ButtonVariant } from '@skyroc/native-ui';
```
...
<UnionType
  name="SlotClassNames"
  description="classNames 的取值形态：把 slot 名映射到类名，每个 slot 都可选。本页用到的 slot 见 ButtonSlots。"
