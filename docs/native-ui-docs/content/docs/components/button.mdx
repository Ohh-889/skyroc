...
- `children` 为 `string` / `number` 时自动包一层 `Text`，样式来自 `TextClassContext`。
- 传入自定义节点时不做包裹，若节点内是 `@skyroc/native-ui` 的 `Text`，同样会继承按钮的文字样式。
- `className` 追加到容器上，`classNames` 按 slot（`root` / `text` / `indicator`）细粒度覆盖，两者都会与内置变体合并。
