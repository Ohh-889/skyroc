...
| `packages/shared/<name>` | `@skyroc/<name>` | `@skyroc/ui-types` |
...
> **新包一律使用 `@skyroc/` scope**，不再使用 `@sa/`（参见 [命名规范](/docs/architecture/naming)）。
...
- 外部依赖一律用 `catalog:<group>`（[Catalog 体系](/docs/architecture/catalog)）；
...
记得**同步更新 `package.json` 的 `exports`**（[Exports 策略](/docs/engineering/exports-strategy)）。
...
按 [开发约定](/docs/engineering/conventions) 编写：
...
或集成到 `sa release` 流程，由 [Git 提交与发版](/docs/engineering/git-commit) 描述。
