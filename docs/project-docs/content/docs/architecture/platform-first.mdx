...
| 业务相关 | 是项目长期沉淀的领域能力 |
| 适配器 | 平台差异通过 [Adapter Pattern](/docs/architecture/adapter-pattern) 注入 |
...
包：`ui-types`（共享 TS 类型）、`type-utils`（高级 TS 工具类型）、`hooks`（跨端 React Hook 集合）。

详见 [Shared 总览](/docs/shared/overview)。
...
具体可视化关系见 [依赖图](/docs/architecture/dependency-graph)。
...
| `shared/<name>` | `@skyroc/<name>` | `@skyroc/ui-types` |
...
## 与 [包分层](/docs/architecture/package-layering) / [Adapter Pattern](/docs/architecture/adapter-pattern) 的关系
