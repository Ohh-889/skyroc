 ...
 * 折叠面板项样式变体。
 *
 * `measure` 槽让内容绝对定位、脱离文档流独立测量，外层高度始终由动画值驱动， 因此 `wrapper` 必须裁掉溢出，否则收起状态下内容仍会露在外面。
 *
 * `content` 是 View，只承载间距与背景；文字类必须放在 `contentText` 上， 因为 RN 不会把文字样式从 View 继承给子级 Text。
 *
...
export const collapseItemVariants = tv({
  ...
  slots: {
    ...
    contentText: 'text-muted-foreground',
