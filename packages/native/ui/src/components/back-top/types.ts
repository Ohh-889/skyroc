 ...
 * 回到顶部按钮属性。
 *
 * 位置与可视边界都按父容器的实测尺寸计算，坐标系约定同 FloatingButton：挂在哪个容器里就贴哪个容器的边。
 */
interface BackTopProps<TRef extends BackTopScrollable = Animated.ScrollView> {
  /**
   * 距父容器底边的距离（像素），默认 128。
   *
   * 不含安全区——位置最终落在 transform 上，拿不到 `*-safe` 工具类背后的运行时 inset。 需要避开 home indicator 或常驻 TabBar 时，由调用方把相应高度并进这个值。
   */
  bottom?: number;
