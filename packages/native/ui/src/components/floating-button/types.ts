...
/** 距屏幕边缘的最小留白；给数字表示两轴相同，给对象可分别指定，缺省的一边回落到默认值 */
type FloatingButtonGap = number | { x?: number; y?: number };

/**
 * 悬浮按钮属性。
 *
 * ⚠️ 坐标系约定：边界按**窗口**尺寸算（`useWindowDimensions`），渲染却是相对父容器的 `absolute`。 因此本组件必须挂在铺满屏幕、且左上角与屏幕左上角重合的容器里（通常是页面根 `View`
 * 的直接子节点）， 否则可视边界会整体偏移。需要盖在所有内容之上时，把它放在页面根节点的最后一个子节点。
 */
interface FloatingButtonProps {
   ...
   * 距屏幕边缘的最小留白，默认 24。
   *
   * 拖拽与受控 `offset` 都会被夹进 `[gap, 窗口尺寸 - size - gap]`。 需要贴边或自行管理边距时传 `0`。
   */
