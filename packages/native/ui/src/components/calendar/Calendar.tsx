 ...
 *
 * 第三方的默认类名里同时带着排版（`p-0.5`、`rounded-md`）和配色，整段替换意味着调用方 只想换个圆角就得把排版重新抄一遍。走 cn 之后冲突的工具类由 tailwind-merge 决定胜负， 不冲突的部分保留。
 */
 ...
 *
 * 之所以只做薄薄一层：它自带的默认类名用的就是 `bg-primary` / `text-primary-foreground` / `text-muted-foreground` / `bg-accent` 这套语义
 * token，与本仓库 tailwind-plugin 生成的 变量同名，配色天然对得上，不需要再翻译一遍。这一层负责的是面板容器样式与类名合并策略。
 */
