 ...
 * 当前蓝牙状态，本模块所有判断的唯一来源。
 *
 * iOS 上首次调用（还没问过蓝牙权限时）返回 `unknown` 且**不会弹窗**——
 * 想要确定值先调一次 {@link requestBluetoothPermission}，或者直接用
 ...
 * 本机有没有蓝牙硬件。
 *
 * 只有明确取不到适配器时才是 `false`（Android 模拟器、iOS `unsupported`），
 * 权限未决 / 蓝牙关着都算「支持」。
 */
 ...
 *
 * 等价于 `state === 'on'`——权限没给时 Android 读不到状态、iOS 直接是 `unauthorized`，
 * 都到不了 `on`，所以这一个判断就够了。
 */
 ...
 *
 * 系统只允许弹一次，之后再调直接返回既有状态：
 * `granted` 为 `false` 且 `canAskAgain` 为 `false` 时就别再调了，
 * 改用 {@link openAppSettings} 引导用户手动开。
 */
 ...
 * - iOS 系统不允许 App 代开蓝牙，跳到本 App 设置页，`handledBy: 'settings'`、
 *   `enabled` 恒为 `false`，需要在用户切回来后自己复查一次状态
 *
 * 所以 **`ok: true` 不等于蓝牙已经开了**，永远看 `payload.enabled`，
 * 或者干脆在 {@link addBluetoothStateListener} 里等状态变成 `on`。
 */
 ...
 * 系统定位服务开没开。
 *
 * **只对 Android 有意义**：Android 12 以下做 BLE 扫描要求定位服务是开的，
 * 关着的话扫描不报错、但一个设备都扫不到。**iOS 恒返回 `true`**。
 */
 ...
 * 监听蓝牙开关状态变化。
 *
 * 原生侧是**有人监听才注册广播 / CBCentralManager，没人监听就注销**，
 * 所以只要记得 `remove()` 就不会有额外开销。
 *
 * ```ts
 * useEffect(() => {
...
function blocked(
  blockedBy: BluetoothReadyOutcome['blockedBy'],
 ...
 *
 * 纯 JS 的组合，没有对应的原生方法，看不惯这个顺序就自己用上面的原子方法拼。
 * **不会 reject**，卡住时通过 `blockedBy` 告诉调用方该提示什么：
 *
 * | `blockedBy` | 该做什么 |
 * | --- | --- |
 ...
 *
 * iOS 上返回 `bluetoothOff` 时**已经跳到设置页**，调用方通常配合
 * {@link addBluetoothStateListener} 或 `AppState` 在用户回来时重跑一次本函数。
 *
 * @param options.requestEnable 蓝牙没开时是否主动引导开启，默认 `true`。
 *   传 `false` 就只做检查不打扰用户（比如启动时的静默探测）
 */
export async function ensureBluetoothReady(
  options?: { requestEnable?: boolean },
): Promise<BluetoothReadyOutcome> {
