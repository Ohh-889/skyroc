...
/**
 * 少了这条 iOS 会在第一次实例化 CBCentralManager 时直接崩溃（不是弹窗失败，是 crash），
 * 所以给一个中文兜底文案，别让忘配的人在真机上才发现。
 */
...
/**
 * Android 侧什么都不用做：权限和 uses-feature 都写在模块自己的
 * `android/src/main/AndroidManifest.xml` 里，prebuild 时由 manifest merger 合进应用。
 *
 * @param {object} props
 * @param {string} [props.bluetoothAlwaysPermission] iOS 蓝牙权限弹窗的说明文案。
 *   会被原样展示给用户，也会被 App Store 审核，写清楚「用来做什么」，别写「需要蓝牙权限」这种废话
 */
