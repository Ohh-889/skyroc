 ...
 *
 * `unknown` 在 iOS 上是**暂时**的：表示「还没问过用户，不主动弹窗所以查不到」， 调一次 `requestBluetoothPermission` 之后再查就会变成确定值。
 */
 ...
 *
 * - **Android 12(S) 以下**没有蓝牙运行时权限（`BLUETOOTH` / `BLUETOOTH_ADMIN` 是安装期授予）， 恒为 `{ granted: true, status: 'granted',
 *   canAskAgain: false }`
...
export type BluetoothPermission = {
   ...
   * 还能不能再弹一次系统弹窗。
   *
   * 为 `false` 且 `granted` 为 `false` 时，只剩「引导用户去设置页手动开」这一条路， 用 `openAppSettings`。
   */
  canAskAgain: boolean;
  granted: boolean;
 ...
 *
 * 理由和 `@skyroc/expo-wechat` 的 `WechatResult` 一致：「用户点了取消」是正常分支 而不是异常，逼调用方 try/catch 再比字符串是错的。
 *
 * 这是个**闭合的判别联合**：两个分支的字段完全一致，缺的那些是 `null` 而不是不存在， 所以既能直接 `JSON.stringify` 上报，又能靠 `if (result.ok)` 收窄：
 *
 * ```ts
 ...
 * - `settings`：iOS 已经跳到本 App 的设置页。**`enabled` 恒为 `false`**—— 系统不会告诉我们用户在设置里做了什么，得等用户切回来后自己复查一次状态 （监听 `AppState` 变成
 *   `active`，或直接用 `addBluetoothStateListener`）
 */
...
export type BluetoothModuleEvents = {
   ...
   * 蓝牙开关状态变化。
   *
   * Android 来自 `BluetoothAdapter.ACTION_STATE_CHANGED` 广播， iOS 来自 `centralManagerDidUpdateState`。
   *
   * 两端都是**有人监听才注册、没人监听就注销**（`OnStartObserving` / `OnStopObserving`）， 不需要手动 register / unregister。
   */
