...
/**
 * 微信在 App Store 的安装地址，由 SDK 给出。
 * 引导「去安装微信」时用它，别自己写死链接。
 */
 ...
 *
 * 会依次检查参数、系统版本、微信版本、SDK 内部操作、拉起微信、从微信返回六步，
 * 中间**会真的切到微信再跳回来**，所以只能在真机上跑。
 * 失败时 `steps` 的最后一条就是卡住的地方，`suggestion` 是微信给的修正建议。
 *
 ...
 * 调整模块的全局行为，返回实际生效的配置。
 *
 * `resumeGraceMs`：从微信切回本 App 后，再等多久还没收到回调就判定这次请求没有结果
 * （返回 `ERR_WECHAT_NO_RESPONSE` 并释放槽位）。默认 **500ms**，会被钳制到 [100, 5000]。
 *
 * 需要调大的情况：Universal Link 走了较慢的跳转链路，实测回调经常晚于 500ms 到达，
 * 此时默认值会把正常结果误判成「没有结果」。
 * 需要调小的情况：确认回调很快，希望用户放弃操作后更快拿到结果。
 *
 * 这是环境属性而不是单次请求的属性（回调延迟跟你分享什么没关系），
 * 所以做成全局配置，而不是给九个 `share*` 各加一个参数。
 * 一般在 App 启动时调一次即可。
 */
...
export type WechatLaunchOptions = {
   ...
   *
   * 在此之前是**本地准备**（下载媒体、压缩缩略图），可能几百毫秒到几秒；
   * 在此之后是**用户在微信里操作**，时长完全不可控。
   * 典型用法是切换 loading 文案：`准备中…` → `等待微信…`
   *
 ...
 * 把「第一段事件」接到这次调用上。
 *
 * 同一种 kind 同时只允许一个在途请求（原生侧用 pending 槽保证），
 * 所以这里按 kind 过滤就足够对上号，不需要额外的请求 id。
 */
 ...
 * 拉起微信授权登录。
 *
 * **永远不会 reject**：成功是 `{ ok: true, payload }`（`payload.code` 拿去给后端换
 * `access_token` / `openid`，换取 token 要用 AppSecret，绝不能放在客户端）；
 * 用户取消 / 拒绝、没装微信、插件没配都是 `{ ok: false, code }`，见 `WechatResultCode`。
 */
 ...
 *
 * App 被系统杀掉后从微信返回时，原生先于 JS 拿到结果，此时没有挂起的 Promise，
 * 结果（含用户取消这种失败）会被缓存在原生侧，需要 JS 起来后主动取一次。
 */
...
export function shareText(
  options: ShareTextOptions & WechatLaunchOptions,
...
export function shareFile(
  options: ShareFileOptions & WechatLaunchOptions,
): Promise<WechatResult<WechatShareResponse>> {
  const { onLaunched, ...rest } = options;
...
export function addWechatRequestSentListener(
  listener: (event: WechatRequestSentEvent) => void,
): EventSubscription {
 ...
 * 监听所有微信响应（第二段）。
 *
 * 注意这是全量通道，正常流程对应方法的 Promise 已经能拿到结果，
 * 两者会同时触发，别重复处理。
 */
export function addWechatResponseListener(
  listener: (result: WechatResult) => void,
): EventSubscription {
 ...
 *
 * 覆盖两种 code：`ERR_WECHAT_USER_CANCELLED`（在微信里明确点了取消）
 * 和 `ERR_WECHAT_NO_RESPONSE`（切回来后没等到回调，多半是按 Home 放弃了）。
 * 两者对用户是一回事，但排查问题时要看原始 `result.code` 区分——
 * `ERR_WECHAT_NO_RESPONSE` 大量出现通常意味着 Universal Link 配置有问题。
 */
