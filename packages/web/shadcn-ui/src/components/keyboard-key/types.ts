...
/**
 * Predefined keyboard keys that have built-in styling and special handling. These keys are commonly used in keyboard
 * shortcuts and key combination displays.
 */
export type KbdKey =
  | 'alt'
  | 'arrowdown'
...
/**
 * Props for the KeyboardKey component. Renders a visual representation of one or more keyboard keys.
 *
 * @example
 *   // Single key
...
export interface KeyboardKeyProps extends Omit<HTMLComponentProps<'kbd'>, 'children'> {
   ...
   */
  symbolize?: boolean;
  /** The keyboard key(s) to display. Can be a single key or an array of keys. */
...
/**
 * Props for the KeyboardKeyGroup component. Renders multiple keyboard keys with a separator between them.
 *
 * @example
...
export interface KeyboardKeyGroupProps extends Omit<KeyboardKeyProps, 'value'> {
  /** Content to display between keyboard keys (separator). Defaults to a visual separator if not provided. */
  separator?: ReactNode;
