...
/**
 * Props for the Skeleton component. A placeholder component that displays an animated loading skeleton while content is
 * loading. Inherits all standard HTML div attributes and custom component props.
 *
 * @example
 *   ```tsx
...
export interface SkeletonProps extends StyledComponentProps<ComponentPropsWithoutRef<'div'>> {
  /**
   * Whether to show skeleton loading state if true, the component will be skeletonized we will hide the content and
...
/**
 * Props for the SkeletonContainer component. A container component that automatically converts children to skeleton
 * placeholders when loading. Preserves the structure and layout of children while showing loading state.
 *
 * @example
 *   ```tsx
...
export interface SkeletonContainerProps extends SkeletonProps {
  ...
  /**
   * Depth level for recursive skeletonization Set to 0 to only skeletonize direct text content
   *
   * @default Infinity
   */
  depth?: number;
  ...
  /**
   * Keys of elements to exclude from skeletonization. Elements with matching keys will remain visible during loading.
