...
/**
 * Props for the drawer content component. The main container that slides in from a specified side of the screen.
 *
 * @example
 *   ```tsx
...
/**
 * Class names for different slots in the drawer component. Allows customizing styles for the overlay, content, and
 * other parts.
 */
export type DrawerClassNames = Partial<Record<DrawerSlots, ClassValue>>;

/**
 * Props for the main Drawer component. A side panel modal that slides in from the edge of the screen.
 *
 * @example
 *   ```tsx
 *   <Drawer scrollable>
...
export interface DrawerProps extends Omit<DialogProps<DrawerContentProps>, 'contentComponent'> {
  /**
   * Whether the drawer content should be scrollable. When true, adds overflow-auto to the content wrapper.
   *
   * @default true
   */
  scrollable?: boolean;
