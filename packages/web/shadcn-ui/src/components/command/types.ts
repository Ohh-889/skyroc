...
/**
 * Class names for different slots in the command component. Allows customizing styles for specific parts of the command
 * palette.
 */
export type CommandClassNames = Partial<Record<CommandSlots, ClassValue>>;

/** Props for the command empty state component. Displayed when no items match the search input. */
export type CommandEmptyProps = StyledComponentProps<ComponentPropsWithoutRef<typeof CommandEmpty>>;

/** Props for the command group component. Groups related command items together with a label. */
...
/** Props for the command input component. The search input field at the top of the command palette. */
...
/** Props for individual command item component. Represents a single selectable command in the list. */
export type CommandItemProps = StyledComponentProps<ComponentPropsWithoutRef<typeof CommandItem>> & {
  /**
   * Keyboard shortcut associated with this command. Can be a single string or array of modifier combinations.
   *
   * @example
   *   'Cmd+K' or ['Ctrl+N', 'Alt+N']
   */
  shortcut?: string | string[];
} & SlotProps;

/** Props for the command list component. Container for all command items and groups. */
export type CommandListProps = StyledComponentProps<ComponentPropsWithoutRef<typeof CommandList>>;

/** Props for the command root/container component. The main wrapper component for the command palette. */
export type CommandRootProps = StyledComponentProps<ComponentPropsWithoutRef<typeof CommandRoot>>;

/** Props for the command separator component. Visual divider between command groups or sections. */
export type CommandSeparatorProps = StyledComponentProps<ComponentPropsWithoutRef<typeof CommandSeparator>>;

/** Props for displaying keyboard shortcuts. Shows the shortcut key combination for a command item. */
export type CommandShortcutProps = HTMLComponentProps<'div'> & {
  /** The shortcut key combination to display. Can be a single string or array of combinations. */
  value?: string | string[];
};

/**
 * Data structure for a command group option. Groups multiple command items together with a label.
 *
 * @example
 *   ```tsx
 *   {
...
/** Data structure for a command separator option. Creates a visual divider in the command list. */
...
/**
 * Data structure for a command item option. Represents a single selectable command in the palette.
 *
 * @example
 *   ```tsx
 *   {
...
/** Union type for command option data. Can be a group, separator, or individual item. */
export type CommandOptionData = CommandGroupOptionProps | CommandSeparatorOptionProps | CommandItemOptionProps;

/** Props for the command option renderer component. Used when rendering individual command options. */
...
/**
 * Props for the main Command component. A searchable command palette/menu with data-driven rendering.
 *
 * @example
 *   ```tsx
 *   <Command
...
/**
 * Props for the Command Dialog component. A dialog-wrapped command palette for modal interaction.
 *
 * @example
 *   ```tsx
 *   <CommandDialog
