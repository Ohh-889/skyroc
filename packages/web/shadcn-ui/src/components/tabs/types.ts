...
/**
 * Represents the visual style properties of the indicator. Used to position and size the active tab indicator element.
 *
 * @example
 *   ```tsx
...
export interface IndicatorStyle {
  ...
  /** The width (in pixels) of the indicator element. Null when no indicator is visible. */
  size: number | null;
}

/**
 * Class names for different slots in the tabs component. Allows customizing styles for specific parts of the tabs
 * (root, triggers, content, indicator, etc.).
 *
 * @example
 *   ```tsx
...
/** Props for the tabs root component. Configures the main tabs container with fill style options. */
export interface TabsRootProps extends StyledComponentProps<_TabsRootProps> {
  /**
   * Determines how the tabs fill the available space. Controls the layout style (e.g., fixed width, expand to fill,
   * etc.).
   */
  fill?: TabsFill;
}
...
/**
 * Props for the tabs list component. Wraps the individual tab triggers and manages the indicator.
 *
 * @example
 *   ```tsx
 *   <TabsList
...
export interface TabsListProps extends StyledComponentProps<_TabsListProps>, Pick<_TabsRootProps, 'value'> {
  ...
  /** Whether to display an animated indicator under the active tab. Defaults to false. */
...
/** Props for the tabs trigger component. Represents a single clickable tab button. */
export interface TabsTriggerProps extends StyledComponentProps<Omit<_TabsTriggerProps, 'className' | 'type'>> {
  /**
   * Whether to display an animated indicator under this trigger when active. Overrides the list-level indicator setting
   * if specified.
   */
  enableIndicator?: boolean;
  /** The visual style type of the tabs. */
...
/**
 * Props for the tabs content component. Wraps the content displayed when a tab is active.
 *
 * @example
 *   ```tsx
 *   <TabsContent
...
/**
 * Data structure for a single tabs item when using the data-driven approach. Use this type to define items passed to
 * the `items` prop of the Tabs component.
 *
 * @example
 *   ```tsx
...
export type TabsOptionData = Pick<TabsTriggerProps, 'disabled'> & {
  /**
   * The content to display when this tab is active. Can be a static ReactNode or a render function that receives active
   * state and item data.
   */
  children: React.ReactNode;
  /** The label/title displayed in the tab trigger button. */
  label: React.ReactNode;
  /** Unique identifier for this tab. Used to associate triggers with content and maintain tab state. */
  value: string;
};

/**
 * Props for the main Tabs component. Supports data-driven rendering via the `items` prop.
 *
 * @example
 *   ```tsx
 *   const items = [
 ...
 */
export interface TabsProps<T extends TabsOptionData = TabsOptionData>
  extends TabsRootProps, Pick<TabsListProps, 'enableIndicator' | 'loop' | 'orientation' | 'shape' | 'type'> {
  ...
  enableIndicator?: boolean;
  /**
   * Whether to force mount all tab content even when inactive. Useful for preserving component state across tab
   * switches.
   */
  forceMountContent?: true;
  /** Array of tab items to render. Each item should have a unique `value` property. */
  items: T[];
