...
/**
 * Class names for different slots in the number input component. Allows customizing styles for the root, buttons,
 * control, and clearable parts.
 */
export type NumberInputClassNames = Partial<Record<NumberInputSlots, ClassValue>>;

/** Props for the NumberInput control component. The actual input element for numeric value entry. */
export interface NumberInputControlProps extends StyledComponentProps<
  Omit<ComponentPropsWithoutRef<'input'>, 'defaultValue' | 'max' | 'min' | 'value'>
> {
  /**
...
/**
 * Props for the main NumberInput component. A numeric input with increment/decrement buttons.
 *
 * @example
 *   ```tsx
 *   // Basic number input
