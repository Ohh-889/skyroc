...
/**
 * Props for the breadcrumb ellipsis component. Displays a visual indicator (usually "...") when breadcrumb items are
 * hidden.
 */
export type BreadcrumbEllipsisProps = HTMLComponentProps<'span'>;

/** Props for the breadcrumb item component. Represents a single breadcrumb navigation item. */
export type BreadcrumbItemProps = HTMLComponentProps<'li'>;

/** Props for the breadcrumb link component. A clickable link in the breadcrumb trail. */
...
/** Props for the breadcrumb list component. Container for all breadcrumb items. */
export type BreadcrumbListProps = HTMLComponentProps<'ol'>;

/** Props for the breadcrumb page component. Displays the current/active page in the breadcrumb trail. */
export type BreadcrumbPageProps = HTMLComponentProps<'span'>;

/** Props for the breadcrumb root component. Main navigation element for breadcrumb navigation. */
export type BreadcrumbRootProps = HTMLComponentProps<'nav'>;

/** Props for the breadcrumb separator component. Displays a visual divider between breadcrumb items (e.g., "/", ">"). */
export type BreadcrumbSeparatorProps = HTMLComponentProps<'li'>;

/** Data structure for a single breadcrumb item. Use this type to define items passed to the Breadcrumb component. */
...
/**
 * Class names for different slots in the breadcrumb component. Allows customizing styles for specific parts of the
 * breadcrumb.
 */
export type BreadcrumbUi = Partial<Record<BreadcrumbSlots, ClassValue>>;

/**
 * Props for the main Breadcrumb component. Manages rendering and navigation of breadcrumb items with optional ellipsis
 * support.
 *
 * @example
 *   ```jsx
...
export type BreadcrumbProps<T extends BreadcrumbItem> = BreadcrumbRootProps & {
  ...
  /**
   * Configuration for displaying ellipsis when items are hidden. Can be true to auto-show ellipsis when items exceed 4,
   * [start, end] for specific range, or null to disable. When true, ellipsis appears when the item count is greater
   * than 4. [start, end] specifies the range indices where ellipsis should be displayed.
   */
  ellipsis?: true | [number, number] | null;
  /**
   * Props for the dropdown menu that wraps the ellipsis. Use this to customize the dropdown menu behavior and
   * appearance.
   */
