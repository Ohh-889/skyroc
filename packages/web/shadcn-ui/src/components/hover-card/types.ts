...
/**
 * Class names for different slots in the hover card component. Allows customizing styles for specific parts of the
 * hover card.
 */
export type HoverCardClassNames = Partial<Record<HoverCardSlots, ClassValue>>;

/**
 * Props for the main Hover Card component. A floating card that appears when hovering over a trigger element.
 *
 * @example
 *   ```tsx
 *   <HoverCard
...
export type HoverCardProps = StyledComponentProps<_HoverCardProps> & {
  ...
  /**
   * Props to customize the content container of the hover card. Includes positioning, delay, and other content
   * behaviors.
   */
  contentProps?: Omit<HoverCardContentProps, 'children' | 'className'>;
  /** Whether to display the arrow indicator pointing to the trigger element. Defaults to false. */
  showArrow?: boolean;
  /** Element that triggers the hover card to appear. Can be any React element or component. */
  trigger?: React.ReactNode;
};

/** Props for the hover card arrow component. The small arrow/pointer that visually connects the trigger to the card. */
export type HoverCardArrowProps = StyledComponentProps<_HoverCardArrowProps>;

/** Props for the hover card content component. Container for the content displayed in the floating hover card. */
