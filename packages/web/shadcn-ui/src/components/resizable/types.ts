...
/**
 * Class names for different slots in the resizable component. Allows customizing styles for the resize handle and its
 * visual elements.
 */
export type ResizableClassNames = Partial<Record<ResizableSlots, ClassValue>>;

/**
 * Props for the resizable handle component. Creates a draggable separator between panels that can be resized by the
 * user.
 *
 * @example
 *   ```tsx
...
export interface ResizableHandleProps extends StyledComponentProps<ComponentProps<typeof PanelResizeHandle>> {
  ...
  /**
   * Whether to display a visual handle indicator (icon/dots) on the resize separator. When false, only the draggable
   * area is visible.
   */
  withHandle?: boolean;
}
...
/**
 * Props for the resizable panel group component. Contains multiple panels that can be resized by dragging the handles
 * between them.
 *
 * @example
 *   ```tsx
