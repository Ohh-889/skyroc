...
/**
 * 根出口必须保持平台中立：React Native 侧会从 `@skyroc/type-utils` 导入， 一旦这里泄漏 DOM 全局类型，无 DOM lib 的环境就会编译失败。
 *
 * 类型断言无法表达「某个 barrel 没有导出什么」，所以这一层用源码静态检查兜底。
 */
...
function readNeutralSources() {
  return readdirSync(SRC_DIR, { withFileTypes: true })
    ...
    .map(entry => ({ content: readFileSync(join(SRC_DIR, entry.name), 'utf8'), name: entry.name }));
}
