 ...
 *
 * 放在本包是因为这里已经是设计令牌的单一数据源（见 `tokens.ts`）：`ThemeColor` 直接 复用插件生成 CSS 变量时用的 {@link SemanticColorName}，两边不会再各写一份而漂移。
 *
 * 本文件零 import，`./ui` 子入口也不会牵出 tailwindcss / @unocss/core —— 只要类型的 组件库可以放心引它。
 */
 ...
 * 组件的尺寸档位。
 *
 * 刻意不复用 `tokens.ts` 的 {@link FontSizeToken}：那是字号尺度（中档叫 `base`）， 这里是组件尺寸档位（中档叫 `md`），两者的语义和取值都不一样。
 */
