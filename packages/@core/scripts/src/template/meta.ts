 ...
 * 链。这些写法在仓库外一律无解，而 `create-admin` 恰恰要在仓库外跑。
 *
 * 解法是把「解析」和「生成」拆到两个时刻：`sync-admin-template` 在 monorepo 内把所有协议解析成具体值，写进本文件描述的
 * sidecar；`create-admin` 只做替换，不需要 monorepo 在场。
 */
