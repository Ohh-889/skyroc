 ...
 *
 * 逻辑与 `@skyroc/utils/web` 的 `getEventValue` 一致，但刻意不从那个入口引 —— 它会把整个 web 工具集（`window` / `document` /
 * `navigator`）带进类型检查， React Native 这类无 DOM 的宿主一引就报一片。这里只靠鸭子类型判断，不依赖 DOM lib。
 *
