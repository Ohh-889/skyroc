 ...
 *
 * 不敏感的本地偏好（主题、引导页看没看过、列表排序……）都走这里。凭据仍然只能进 SECURE_STORAGE，
 * MMKV 是明文文件，root 过的设备直接能读。
 */
export const MMKV_STORAGE = 'mmkv';

/**
 * 显式给一个 id，不用默认的 `mmkv.default`：默认实例是全库共用的，第三方库（很多都依赖 MMKV）
 * 往里写同名 key 就会互相覆盖。
 */
const storage = createMMKV({ id: 'app' });

/**
 * MMKV 是 mmap + JSI 的同步读写，正好对上 core-state 的同步 `AtomStorage` 接口：
 * 冷启动时偏好是**同步**就位的，不会先按默认值画一帧再跳成用户选的值。
 *
