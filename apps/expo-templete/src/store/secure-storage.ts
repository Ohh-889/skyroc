 ...
 *
 * 凭据这类东西必须存 SecureStore（iOS Keychain / Android Keystore），不能进 AsyncStorage —— 后者是明文文件，root 过的设备直接能读。
 */
export const SECURE_STORAGE = 'secure';

/**
 * Core-state 的 `AtomStorage` 是同步接口，正好对上 SecureStore 的同步读写： 冷启动时凭据是**同步**就位的，不需要 loading 态，也就不会闪一帧登录页。
 *
