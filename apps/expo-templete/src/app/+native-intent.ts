 ...
 *
 * 它对进来的 URL 不做来源过滤（linking 配置里 `prefixes: []`），https 链接会被直接 截成路径去匹配路由。微信在 iOS 上正是用 Universal Link 把 App 拉回前台的，
 * 于是「取消授权」回来时 `https://<universalLink>/?...` 会被解析成一个不存在的路由， 跳到 +not-found。授权结果本身由原生模块的回调送达，不需要走路由，这里直接丢弃。
 */
