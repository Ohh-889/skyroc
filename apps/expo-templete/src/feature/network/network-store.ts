 ...
 * 当前是否有可用网络。
 *
 * 初值给 `true` 而不是 `false`：冷启动时真实状态还没读回来，先当在线，最坏是多发一个立刻失败的请求；
 * 反过来先当断网，会在每次启动时闪一条断网提示，还会让首屏的查询被 `onlineManager` 直接暂停。
 *
 * 和 `authAtom` 一个路子：请求层在 React 之外用 `getIsOnline()` 同步读，界面用 `useIsOnline()`
 * 跟着重渲染，一份状态两条出口。
 */
 ...
 * 由系统上报的网络状态判断能不能发请求。
 *
 * 两个字段都用 `!== false` 而不是真值判断：它们都是可选的，某些设备 / 平台上会是 `undefined`，
 * 那种「不知道」的情况必须当成在线——把未知判成断网，会让整个 App 在这些设备上彻底发不出请求。
 *
 * `isInternetReachable` 也一起看，是为了认出酒店 / 机场那种要先点同意才放行的 WiFi：
 * 连是连上了（`isConnected` 为 true），但所有请求都会被网关劫持。安卓上它要求链路真的通过了
 ...
 *
 * 监听器只在状态**变化**时回调，所以还要主动读一次当前状态打底，否则「一启动就没网」这种最该提示的
 * 情况反而一条提示都不会有。
 */
