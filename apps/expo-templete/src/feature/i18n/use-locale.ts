 ...
 * 读写语言。
 *
 * 和 `useThemeMode` 一个路子，两个来源各管一半：偏好是自己的持久化状态（`localePreferenceAtom`）， 生效的语言由它加上系统语言算出来。UI 上「选中哪一项」看
 * `preference`，「现在在说什么语言」看 `locale`。
 *
 * 这里**不返回 `t`**：取词条请各自 `useTranslation()`，那是 react-i18next 精确订阅的入口， 从这里透传只会让所有用到语言状态的组件在切系统语言时一起重渲染。
 */
