 ...
 *
 * 和 `themeModeAtom` 一模一样的取舍，理由也一样：存**偏好**而不是最终生效的语言，用户把手机语言
 * 改掉之后 App 才会跟着变（见 `types.ts` 里 `LocalePreference` 的注释）。
 *
 * 落盘走 `MMKV_STORAGE`：MMKV 的读是同步的，冷启动第一帧就能拿到偏好。i18next 的初始化因此也能是
 * 同步的（见 `i18n.ts`），首屏不会先用兜底语言画一遍再跳成用户选的语言——那一跳在长文案上是肉眼可见的。
 */
 ...
 * 按系统语言挑一门支持的语言。
 *
 * `getLocales()` 返回的是用户在系统设置里排好序的**列表**，不是单个值——「首选西班牙语、次选英语」
 * 的用户应该看到英文而不是中文兜底，所以要顺着列表往下找，找到第一个能对上的为止。
 *
 * 两级匹配：先按完整 tag（`zh-Hant-TW` 精确命中 `zh-TW`），再按语言码（`zh-Hant-TW` → `zh` → `zh-CN`）。
 * 只做后者的话，将来同时支持简繁体时会把繁体用户送进简体界面。
 *
 ...
 *
 * 偏好是 `system` 时每次都重新问一遍系统——安卓上用户可以在不杀进程的情况下改系统语言，
 * 缓存住这个结果会让 App 停在旧语言上。
 */
