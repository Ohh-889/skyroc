 ...
 * React 之外取词条。
 *
 * 它**不会**跟着语言变化重渲染任何东西——只在调用的那一刻取值。放进模块级常量 （`const TITLE = t('x')`）就等于把首次启动时的语言焊死了，切语言后再也不变。 组件里一律用 `useTranslation()`。
 */
 ...
 *
 * 目的是**只有一个入口**：不管文案是在 JSX 里调 `formatDate` 拼出来的，还是在 JSON 里用插值写的， 走的都是 `feature/i18n/format`
 * 那一份实现。少了这一步，翻译同学一旦在词条里自己拼日期， 格式就和界面其它地方对不上了。
 *
 * I18next 自带的 `relativetime` / `list` 格式化器**不能用**：它们建在 Hermes 没有的 `Intl.RelativeTimeFormat` / `Intl.ListFormat`
 * 上，调用会直接抛。
 */
 ...
 *
 * 关键是 `initImmediate: false`：i18next 默认把初始化推到下一个 tick（为了给异步 backend 让路）， 那样首帧渲染时 `t()`
 * 还什么都取不到，长文案会先空一帧再跳出来。词条本来就是打包进来的， 没有任何要等的东西，所以直接要求同步完成——配合 MMKV 的同步读，第一帧就是用户选的语言。
 *
...
/**
 * 切语言。写偏好和通知 i18next 必须绑在一起，别在页面里分开调：只写偏好的话界面不变， 只调 changeLanguage 的话下次冷启动又跳回去了。
 *
