 ...
 * 两个理由，任一个都够：
 *
 * 1. 打断循环依赖。`request` 创建时要拿到 adapter，adapter 又要拿到续签函数——续签函数一旦 建立在 `request` 上，`api → request → adapter → api` 就成环了（Metro
 *    会直接报 Require cycle，且加载顺序稍有变化就会拿到半初始化的模块）。
 * 2. 续签请求本来就不该走那套拦截器。它带的是已经过期的 token，一旦被判成「令牌过期」， 就会去 await 自己那次还没完成的刷新，把自己和所有排队等刷新的请求一起挂死——不是报错， 是永远转圈。`@skyroc/service`
 *    里那一整套 `isRefreshTokenRequest` 判断防的就是这件事， 走独立通道则从根上不可能发生。
 *
 ...
 *
 * 只给 `service/adapter` 用。业务代码需要刷新令牌时走 `refreshToken(adapter)`——那里有并发去重， 各刷各的会让后发的那次拿着已经轮换掉的 refreshToken
 * 去换，换回来一次失败和一次莫名其妙的登出。
 *
