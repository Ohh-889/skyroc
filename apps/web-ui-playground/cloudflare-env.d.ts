...
/**
 * An abnormal event (called an exception) which occurs as a result of calling a method or accessing a property of a web
 * API.
 *
...
/**
 * This ServiceWorker API interface represents the global execution context of a service worker. Available only in
 * secure contexts.
 *
...
/**
 * The Web Crypto API provides a set of low-level functions for common cryptographic tasks. The Workers runtime
 * implements the full surface of this API, but with some differences in the [supported
 * algorithms](https://developers.cloudflare.com/workers/runtime-apis/web-crypto/#supported-algorithms) compared to
 * those implemented in most browsers.
 *
...
/**
 * The Workers runtime supports a subset of the Performance API, used to measure timing and performance, as well as
 * timing of subrequests and other operations.
 *
...
declare class Event {
  ...
  /**
   * Returns true or false depending on how event was initialized. True if event invokes listeners past a ShadowRoot
   * node that is the root of its target, and false otherwise.
   *
   ...
   * true can indicate that part of the operation during which event was dispatched, can be canceled by invoking the
  ...
  /**
   * Returns the invocation target objects of event's path (objects on which listeners will be invoked), except for any
   * nodes in shadow trees of which the shadow root's mode is "closed" that are not reachable from event's
   * currentTarget.
   *
...
declare class AbortController {
  ...
  /**
   * Invoking this method will set this object's AbortSignal's aborted flag and signal to any observers that the
...
/**
 * Extends the lifetime of the install and activate events dispatched on the global scope as part of the service worker
 * lifecycle. This ensures that any functional events (like FetchEvent) are not dispatched until it upgrades database
...
/**
 * This Web Crypto API interface provides a number of low-level cryptographic functions. It is accessed via the
 * Crypto.subtle properties available in a window context (via Window.crypto). Available only in secure contexts.
 *
...
/**
 * A decoder for a specific method, that is a specific character encoding, like utf-8, iso-8859-2, koi8, cp1261, gbk,
 * etc. A decoder takes a stream of bytes as input and emits a stream of code points. For a more scalable, non-native
 * library, see StringView – a C-like representation of strings based on typed arrays.
 *
...
declare class TextDecoder {
   ...
   * stream set to true, and then once without options's stream (or set to false), to process a fragmented input. If the
   * invocation without options's stream (or set to false) has no input, it's clearest to omit both arguments.
   *
   *     var string = '',
...
/**
 * TextEncoder takes a stream of code points as input and emits a stream of bytes. For a more scalable, non-native
 * library, see StringView – a C-like representation of strings based on typed arrays.
 *
...
declare class MessageEvent extends Event {
  ...
  /**
   * Returns the WindowProxy of the source window, for cross-document messaging, and the MessagePort being attached, in
   * the connect event fired at SharedWorkerGlobalScope objects.
   *
...
/**
 * Provides a way to easily construct a set of key/value pairs representing form fields and their values, which can then
 * be easily sent using the XMLHttpRequest.send() method. It uses the same format a form would use if the encoding type
...
interface StreamPipeOptions {
   ...
   *
   * An error in this source readable stream will abort destination, unless preventAbort is truthy. The returned promise
   * will be rejected with the source's error, or with any error that occurs during aborting the destination.
   *
...
interface ReadableStreamGetReaderOptions {
   ...
   *
   * This call behaves the same way as the no-argument variant, except that it only works on readable byte streams, i.e.
   * streams which were constructed specifically with the ability to handle "bring your own buffer" reading. The
   * returned BYOB reader provides the ability to directly read individual chunks from the stream via its read() method,
...
/**
 * This Streams API interface represents a controller allowing control of a WritableStream's state. When constructing a
 * WritableStream, the underlying sink is given a corresponding WritableStreamDefaultController instance to manipulate.
 *
...
interface ReadableWritablePair<R = any, W = any> {
   ...
   * writable, readable } pair). It simply pipes the stream into the writable side of the supplied pair, and returns the
   * readable side for further use.
   *
   * Piping a stream will lock it for the duration of the pipe, preventing any other consumer from acquiring a reader.
   */
...
/**
 * This Streams API interface provides a built-in byte length queuing strategy that can be used when constructing
 * streams.
 *
...
interface QueuingStrategyInit {
   ...
   *
   * Note that the provided high water mark will not be validated ahead of time. Instead, if it is negative, NaN, or not
   * a number, the resulting ByteLengthQueuingStrategy will cause the corresponding stream constructor to throw.
   */
...
/**
 * Provides the API for creating and managing a WebSocket connection to a server, as well as for sending and receiving
...
interface WebSocket extends EventTarget<WebSocketEventMap> {
  ...
  url: string | null;
  /**
   * Returns the subprotocol selected by the server, if any. It can be used in conjunction with the array form of the
...
declare class EventSource extends EventTarget {
  ...
  /**
   * Aborts any instances of the fetch algorithm started for this EventSource object, and sets the readyState attribute
   * to CLOSED.
   *
  ...
  get url(): string;
  /**
   * Returns true if the credentials mode for connection requests to the URL providing the event stream is set to
...
/**
 * This Channel Messaging API interface represents one of the two ports of a MessageChannel, allowing messages to be
...
interface MessagePort extends EventTarget {
  /**
   * Posts a message through the channel. Objects listed in transfer are transferred, not just cloned, meaning that they
   * are no longer usable on the sending side.
   *
   * Throws a "DataCloneError" DOMException if transfer contains duplicate objects or port, or if message could not be
   * cloned.
   *
...
type Ai_Cf_Baai_Bge_Base_En_V1_5_Input =
  | {
      text: string | string[];
      /**
       * The pooling method used in the embedding process. `cls` pooling will generate more accurate embeddings on
       * larger inputs - however, embeddings created with cls pooling are not compatible with embeddings generated with
       * mean pooling. The default pooling method is `mean` in order for this to not be a breaking change, but we highly
  ...
  | {
      ...
      requests: {
        ...
        /**
         * The pooling method used in the embedding process. `cls` pooling will generate more accurate embeddings on
         * larger inputs - however, embeddings created with cls pooling are not compatible with embeddings generated
         * with mean pooling. The default pooling method is `mean` in order for this to not be a breaking change, but we
         * highly suggest using the new `cls` pooling for better accuracy.
         */
        pooling?: 'mean' | 'cls';
      }[];
    };
...
type Ai_Cf_Unum_Uform_Gen2_Qwen_500M_Input =
  ...
  | {
      ...
      raw?: boolean;
      /**
       * Controls the creativity of the AI's responses by adjusting how many possible words it considers. Lower values
...
interface Prompt {
  ...
  temperature?: number;
  /**
   * Adjusts the creativity of the AI's responses by controlling how many possible words it considers. Lower values make
...
interface Ai_Cf_Deepgram_Nova_3_Input {
  ...
  /**
   * Opts out requests from the Deepgram Model Improvement Program. Refer to our Docs for pricing impacts before setting
   * this to true. https://dpgr.am/deepgram-mip.
   */
  mip_opt_out?: boolean;
   ...
   * processed time range and returns the transcript with a speech_final parameter set to true. Can also be set to false
   * to disable endpointing
   */
  endpointing?: string;
  /**
...
interface GPT_OSS_120B_Responses_Async {
  requests: {
    ...
    reasoning?: {
       ...
       * model's reasoning process. One of auto, concise, or detailed.
       */
      summary?: 'auto' | 'concise' | 'detailed';
    };
  }[];
...
interface Ai_Cf_Deepgram_Aura_1_Input {
  ...
  sample_rate?: number;
  /**
   * The bitrate of the audio in bits per second. Choose from predefined ranges or specific values based on the encoding
   * type.
   */
  bit_rate?: number;
}
...
interface BasicImageTransformations {
  ...
  height?: number;
  /**
   * Resizing mode as a string. It affects interpretation of width and height options: - scale-down: Similar to contain,
   * but the image is never enlarged. If the image is larger than given width or height, it will be resized. Otherwise
   * its original size will be kept. - contain: Resizes to maximum size that fits within the given width and height. If
   * only a single dimension is given (e.g. only width), the image will be shrunk or enlarged to exactly match that
   * dimension. Aspect ratio is always preserved. - cover: Resizes (shrinks or enlarges) to fill the entire area of
   * width and height. If the image has an aspect ratio different from the ratio of width and height, it will be cropped
   * to fit. - crop: The image will be shrunk and cropped to fit within the area specified by width and height. The
   * image will not be enlarged. For images smaller than the given dimensions it's the same as scale-down. For images
   * larger than the given dimensions, it's the same as cover. See also trim. - pad: Resizes to the maximum size that
   * fits within the given width and height, and then fills the remaining area with a background color (white by
   * default). Use of this mode is not recommended, as the same effect can be more efficiently achieved with the contain
   * mode and the CSS object-fit: contain property. - squeeze: Stretches and deforms to the width and height given, even
   * if it breaks aspect ratio
   */
  fit?: 'scale-down' | 'contain' | 'cover' | 'crop' | 'pad' | 'squeeze';
  /**
   * Image segmentation using artificial intelligence models. Sets pixels not within selected segment area to
   * transparent e.g "foreground" sets every background pixel as transparent.
   */
  segment?: 'foreground';
  /**
   * When cropping with fit: "cover", this defines the side or point that should be left uncropped. The value is either
   * a string "left", "right", "top", "bottom", "auto", or "center" (the default), or an object {x, y} containing focal
   * point coordinates in the original image expressed as fractions ranging from 0.0 (top or left) to 1.0 (bottom or
   * right), 0.5 being the center. {fit: "cover", gravity: "top"} will crop bottom or left and right sides as necessary,
   * but won’t crop anything from the top. {fit: "cover", gravity: {x:0.5, y:0.2}} will crop each side to preserve as
   * much as possible around a point at 20% of the height of the source image.
   */
  gravity?:
    | 'face'
    | 'left'
  ...
  /**
   * Background color to add underneath the image. Applies only to images with transparency (such as PNG). Accepts any
   * CSS color (#RRGGBB, rgba(…), hsl(…), etc.)
   */
  background?: string;
 ...
 * constructor, you can set certain properties of a `cf` object to control how Cloudflare features are applied to that
 * new Request.
 *
...
interface RequestInitCfProperties extends Record<string, unknown> {
  ...
  /**
   * A request's cache key is what determines if two requests are "the same" for caching purposes. If a request has the
   * same cache key as some previous request, then we can serve the same cached response for both. (e.g. 'some-key')
   *
   * Only available for Enterprise customers.
   */
  cacheKey?: string;
  /**
   * This allows you to append additional Cache-Tag response headers to the origin response without modifications to the
   * origin server. This will allow for greater control over the Purge by Cache Tag feature utilizing changes only in
   * the Workers process.
   *
   * Only available for Enterprise customers.
   */
  ...
  cacheTtl?: number;
  /**
   * Force response to be cached for a given number of seconds based on the Origin status code. (e.g. { '200-299':
   * 86400, '404': 1, '500-599': 0 })
   */
  ...
  /**
   * Redirects the request to an alternate origin server. You can use this, for example, to implement load balancing
   * across several origins. (e.g.us-east.example.com)
   *
   * Note - For security reasons, the hostname set in resolveOverride must be proxied on the same Cloudflare zone of the
   * incoming request. Otherwise, the setting is ignored. CNAME hosts are allowed, so to resolve to a host under a
   * different domain or a DNS only domain first declare a CNAME record within your own zone’s DNS mapping to the
   * external hostname, set proxy on Cloudflare, then set resolveOverride to point to that CNAME record.
   */
  resolveOverride?: string;
}
interface RequestInitCfPropertiesImageDraw extends BasicImageTransformations {
  /**
   * Absolute URL of the image file to use for the drawing. It can be any of the supported file formats. For drawing of
   * watermarks or non-rectangular overlays we recommend using PNG or WebP images.
   */
  url: string;
  /**
   * Floating-point number between 0 (transparent) and 1 (opaque). For example, opacity: 0.5 makes overlay
   * semitransparent.
   */
  opacity?: number;
  /**
   * - If set to true, the overlay image will be tiled to cover the entire area. This is useful for stock-photo-like
   *   watermarks.
  ...
  repeat?: true | 'x' | 'y';
  /**
   * Position of the overlay image relative to a given edge. Each property is an offset in pixels. 0 aligns exactly to
   * the edge. For example, left: 10 positions left side of the overlay 10 pixels from the left edge of the image it's
   * drawn over. bottom: 0 aligns bottom of the overlay with bottom of the background image.
   *
...
interface RequestInitCfPropertiesImage extends BasicImageTransformations {
  /**
   * Device Pixel Ratio. Default 1. Multiplier for width/height that makes it easier to specify higher-DPI sizes in <img
   * srcset>.
   */
  dpr?: number;
  /**
   ...
   *
   * It can be used as: - left, top, right, bottom - it will specify the number of pixels to cut off each side - width,
   * height - the width/height you'd like to end up with - can be used in combination with the properties above - border
   * - this will automatically trim the surroundings of an image based on it's color. It consists of three properties: -
   * color: rgb or hex representation of the color you wish to trim (todo: verify the rgba bit) - tolerance: difference
   * from color to treat as color - keep: the number of pixels of border to keep
   */
  trim?:
    | 'border'
    | {
  ...
  /**
   * Quality setting from 1-100 (useful values are in 60-90 range). Lower values make images look worse, but load
   * faster. The default is 85. It applies only to JPEG and WebP images. It doesn’t have any effect on PNG.
   */
  quality?: number | 'low' | 'medium-low' | 'medium-high' | 'high';
  /**
   * Output format to generate. It can be: - avif: generate images in AVIF format. - webp: generate images in Google
   * WebP format. Set quality to 100 to get the WebP-lossless format. - json: instead of generating an image, outputs
   * information about the image, in JSON format. The JSON object will contain image size (before and after resizing),
   * source image’s MIME type, file size, etc. - jpeg: generate images in JPEG format. - png: generate images in PNG
   * format.
   */
  format?: 'avif' | 'webp' | 'json' | 'jpeg' | 'png' | 'baseline-jpeg' | 'png-force' | 'svg';
  /**
   * Whether to preserve animation frames from input files. Default is true. Setting it to false reduces animations to
   * still images. This setting is recommended when enlarging images or processing arbitrary user content, because large
   * GIF animations can weigh tens or even hundreds of megabytes. It is also useful to set anim:false when using
   * format:"json" to get the response quicker without the number of frames.
   */
  anim?: boolean;
  /**
   * What EXIF data should be preserved in the output image. Note that EXIF rotation and embedded color profiles are
   * always applied ("baked in" into the image), and aren't affected by this option. Note that if the Polish feature is
   * enabled, all metadata may have been removed already and this option may have no effect. - keep: Preserve most of
   * EXIF metadata, including GPS location if there's any. - copyright: Only keep the copyright tag, and discard
   * everything else. This is the default behavior for JPEG files. - none: Discard all invisible EXIF metadata.
   * Currently WebP and PNG output formats always discard metadata.
   */
  metadata?: 'keep' | 'copyright' | 'none';
  /**
   * Strength of sharpening filter to apply to the image. Floating-point number between 0 (no sharpening, default) and
   * 10 (maximum). 1.0 is a recommended value for downscaled images.
   */
  sharpen?: number;
  ...
  /**
   * Fetching image from authenticated origin. Setting this property will pass authentication headers (Authorization,
   * Cookie, etc.) through to the origin.
   */
  'origin-auth'?: 'share-publicly';
  /**
   * Adds a border around the image. The border is added after resizing. Border width takes dpr into account, and can be
   * specified either using a single width property, or individually for each side.
   */
  border?:
    | {
        color: string;
  ...
  /**
   * Increase brightness by a factor. A value of 1.0 equals no change, a value of 0.5 equals half brightness, and a
   * value of 2.0 equals twice as bright. 0 is ignored.
   */
  brightness?: number;
  /**
   * Increase contrast by a factor. A value of 1.0 equals no change, a value of 0.5 equals low contrast, and a value of
   * 2.0 equals high contrast. 0 is ignored.
   */
  contrast?: number;
  /**
   * Increase exposure by a factor. A value of 1.0 equals no change, a value of 0.5 darkens the image, and a value of
   * 2.0 lightens the image. 0 is ignored.
   */
  gamma?: number;
  /**
   * Increase contrast by a factor. A value of 1.0 equals no change, a value of 0.5 equals low contrast, and a value of
  ...
  saturation?: number;
  /**
   * Flips the images horizontally, vertically, or both. Flipping is applied before rotation, so if you apply
   * flip=h,rotate=90 then the image will be flipped horizontally, then rotated by 90 degrees.
   */
  flip?: 'h' | 'v' | 'hv';
  /**
   * Slightly reduces latency on a cache miss by selecting a quickest-to-compress file format, at a cost of increased
   * file size and lower image quality. It will usually override the format option and choose JPEG over WebP or AVIF. We
   * do not recommend using this option, except in unusual circumstances like resizing uncacheable dynamically-generated
   * images.
   */
  compression?: 'fast';
}
...
interface IncomingRequestCfPropertiesBase extends Record<string, unknown> {
  ...
  clientTcpRtt?: number;
  /**
   * The three-letter [IATA](https://en.wikipedia.org/wiki/IATA_airport_code) airport code of the data center that the
   * request hit.
   *
   * @example
   *   'DFW';
  ...
  /**
   * The TLS version of the connection to Cloudflare. In requests served over plaintext (without TLS), this property is
   * the empty string `""`.
   *
   * @example
   *   'TLSv1.3';
  ...
  tlsVersion: string;
  /**
   * The cipher for the connection to Cloudflare. In requests served over plaintext (without TLS), this property is the
   * empty string `""`.
   *
   * @example
...
interface IncomingRequestCfPropertiesBotManagementBase {
  ...
  score: number;
  /**
   * A boolean value that is true if the request comes from a good bot, like Google or Bing. Most customers choose to
   * allow this traffic. For more details, see [Traffic from known
...
interface IncomingRequestCfPropertiesCloudflareForSaaSEnterprise<HostMetadata> {
   ...
   *
   * This field is only present if you have Cloudflare for SaaS enabled on your account and you have followed the
...
interface IncomingRequestCfPropertiesCloudflareAccessOrApiShield {
   ...
   *
   * This is populated when the incoming request is served over TLS using either Cloudflare Access or API Shield (mTLS)
   * and the presented SSL certificate has a valid [Certificate Serial
   * Number](https://ldapwiki.com/wiki/Certificate%20Serial%20Number) (i.e., not `null` or `""`).
   *
   * Otherwise, a set of placeholder values are used.
   *
   * The property `certPresented` will be set to `"1"` when the object is populated (i.e. the above conditions were
   * met).
   */
...
interface D1Meta {
  ...
  /**
   * Number of total attempts to execute the query, due to automatic retries. Note: All other fields in the response
   * like `timings` only apply to the last attempt.
   */
...
declare abstract class D1Database {
  ...
  /**
   * Creates a new D1 Session anchored at the given constraint or the bookmark. All queries executed using the created
   * session will have sequential consistency, meaning that all writes done through the session will be visible in
   * subsequent reads.
   *
...
declare abstract class D1DatabaseSession {
  ...
  batch<T = unknown>(statements: D1PreparedStatement[]): Promise<D1Result<T>[]>;
  /**
   * @returns The latest session bookmark across all executed queries on the session. If no query has been executed yet,
   *   `null` is returned.
   */
...
interface Hyperdrive {
   ...
   *
   * Calling this method returns an idential socket to if you call `connect("host:port")` using the `host` and `port`
   * fields from this object. Pick whichever approach works better with your preferred DB client library.
   *
   * Note that this socket is not yet authenticated -- it's expected that your code (or preferably, the client library
   * of your choice) will authenticate using the information in this class's readonly fields.
   */
  connect(): Socket;
  /**
   * A valid DB connection string that can be passed straight into the typical client library/driver/ORM. This will
   * typically be the easiest way to use Hyperdrive.
   */
...
interface ImageTransformer {
  /**
   * Apply transform next, returning a transform handle. You can then apply more transformations, draw, or retrieve the
   * output.
   *
   * @param transform
   */
  ...
  /**
   * Draw an image on this transformer, returning a transform handle. You can then apply more transformations, draw, or
   * retrieve the output.
   *
...
/**
 * Media transformer for applying transformation operations to media content. Handles sizing, fitting, and other input
...
/**
 * Generator for producing media transformation results. Configures the output format and parameters for the transformed
 * media.
 */
...
/**
 * Configuration options for Media Transformations output. Controls the format, timing, and type of the generated
 * output.
 */
...
/**
 * Error object for media transformation operations. Extends the standard Error interface with additional media-specific
 * information.
 */
...
declare module 'cloudflare:pipelines' {
  ...
  > {
    ...
    /**
     * Run recieves an array of PipelineRecord which can be transformed and returned to the pipeline
     *
...
interface SecretsStoreSecret {
  /**
   * Get a secret from the Secrets Store, returning a string of the secret value if it exists, or throws an error if it
   * does not exist
   */
  get(): Promise<string>;
}
...
/** Supported distance metrics for an index. Distance metrics determine how other "similar" vectors are determined. */
 ...
 * Default to "none".
 *
 * @property all Full metadata for the vector return set, including all fields (including those un-indexed) without
 *   truncation. This is a more expensive retrieval, as it requires additional fetching & reading of un-indexed data.
 ...
 *
 * This type is exclusively for the Vectorize **beta** and will be deprecated once Vectorize RC is released. See
 * {@link VectorizeIndexInfo} for its post-beta equivalent.
 */
...
interface VectorizeIndexInfo {
  ...
  dimensions: number;
  /**
   * ISO 8601 datetime of the last processed mutation on in the index. All changes before this mutation will be
   * reflected in the index state.
   */
  processedUpToDatetime: number;
  /**
   * UUIDv4 of the last mutation processed by the index. All changes before this mutation will be reflected in the index
   * state.
   */
...
interface VectorizeVector {
  /**
   * The ID for the vector. This can be user-defined, and must be unique. It should uniquely identify the object, and is
   * best set based on the ID of what the vector represents.
   */
  id: string;
  /** The vector values */
...
/**
 * Results of an operation that performed a mutation on a set of vectors. Here, `ids` is a list of vectors that were
 * successfully processed.
 *
 * This type is exclusively for the Vectorize **beta** and will be deprecated once Vectorize RC is released. See
 * {@link VectorizeAsyncMutation} for its post-beta equivalent.
 */
...
/**
 * Result type indicating a mutation on the Vectorize Index. Actual mutations are processed async where the `mutationId`
 * is the unique identifier for the operation.
 */
 ...
 *
 * This type is exclusively for the Vectorize **beta** and will be deprecated once Vectorize RC is released. See
 * {@link Vectorize} for its new implementation.
 */
...
declare module 'cloudflare:workflows' {
  /**
   * NonRetryableError allows for a user to throw a fatal error that makes a Workflow instance fail immediately without
   * triggering a retry
   */
...
interface WorkflowInstanceCreateOptions<PARAMS = unknown> {
  ...
  params?: PARAMS;
  /**
   * The retention policy for Workflow instance. Defaults to the maximum retention period available for the owner's
   * account.
   */
  retention?: {
