...
- 🛠️ **丰富示例**：内置大量业务组件、插件 demo 与主题配置选项
...
`admin-example` 应用内置了完整的功能 demo 页面，覆盖常见中后台场景：

### 🔌 插件（plugin）

| 页面             | 说明                       |
| ---------------- | -------------------------- |
| charts / echarts | ECharts 图表示例           |
| tables / vtable  | VTable 高性能表格          |
| editor           | 富文本编辑器（WangEditor） |
| map              | Leaflet 地图               |
| pdf              | PDF 预览（react-pdf）      |
| excel            | Excel 导入导出（xlsx）     |
| barcode          | 条形码生成（JsBarcode）    |
| print            | 打印（print-js）           |
| swiper           | 轮播组件（Swiper）         |
| video            | 视频播放（xgplayer）       |
| icon             | 图标浏览与使用             |
| copy             | 剪贴板复制                 |
| pinyin           | 拼音转换（pinyin-pro）     |
| typeit           | 打字机动画（TypeIt）       |
| gantt            | 甘特图（dhtmlx-gantt）     |
...
| multi-tab   | 多标签页管理示例    |
| tab         | 标签页操作          |
| request     | HTTP 请求与错误处理 |
| super-page  | 超级管理员专属页面  |
| toggle-auth | 动态权限切换        |
| hide-child  | 子元素隐藏控制      |
...
| 页面         | 说明                                          |
| ------------ | --------------------------------------------- |
| feedback     | 反馈组件（Modal / Message / Notification 等） |
| notification | 通知公告                                      |
| theme        | 主题配置与切换                                |

### 📚 文档（document）

内置各技术文档站点快捷入口：React、TanStack Query、TanStack Router、Jotai、Ant Design、UnoCSS、Vite、管理系统文档、UI 组件文档、核心包文档、Web Kit 文档等。

### 其他页面

- `home` — 首页仪表盘
- `about` — 关于页面
- `projects` — 项目列表（动态路由示例 `[pid]`）
- `multi-menu` — 多级菜单示例
...
│   ├── admin-example/                   # 含完整 demo 页面的示例应用（本应用）
│   │   ├── src/
│   │   │   ├── assets/                  # 静态资源
│   │   │   ├── components/              # 局部组件（SystemLogo、SkyrocAvatar）
│   │   │   ├── features/                # 功能模块
│   │   │   │   ├── antd/               # Antd 全局配置
│   │   │   │   ├── auth/               # 认证模块
...
│   │   ├── .env.prod                    # 生产环境变量
│   │   ├── .env.test                    # 测试环境变量
│   │   └── vite.config.ts               # Vite 配置（端口 9528）
...
# 方式二：进入应用目录后启动
cd apps/admin-example
pnpm dev
```

开发服务器将在 `http://localhost:9528` 启动
